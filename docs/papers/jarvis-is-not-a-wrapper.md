...
- **`draft-[REDACTED-NDA]-replies-on-behalf`** — when a third party messages and the right move is for me to draft in his voice for his approval (buffer against honesty-leak), not respond directly.
...
- **Filesystem-native CRMs** — `LinkedIn_Queue/` (30-post queue, dashboard, style guide, 7-week schedule, analytics scaffold — built in 30 minutes), `USD8_Queue/` (Rick-facing partnership CRM with single canonical dashboard view), `[REDACTED-NDA]_Reports/` (daily paper trail).
