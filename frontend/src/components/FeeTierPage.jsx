...
function TierProgress({ userVolume, userTier }) {
  ...
  const progress = next
    ? Math.min(1, (userVolume - current.minVolume) / (next.minVolume - current.minVolume))
    : 1
          ...
          <div className="flex-1 min-w-0">
            ...
            <div className="flex gap-3 mt-1.5">
              <div className="flex-1">
                <div className="text-[9px] font-mono text-black-500 mb-0.5">Maker</div>
                <div className="relative h-1.5 rounded-full overflow-hidden" style={{ background: 'rgba(255,255,255,0.06)' }}>
                  <div className="absolute inset-y-0 left-0 rounded-full" style={{
                    width: `${(comp.maker / maxFee) * 100}%`,
                    background: comp.maker === 0 ? '#22c55e' : comp.color,
                  }} />
                </div>
              </div>
              <div className="flex-1">
                <div className="text-[9px] font-mono text-black-500 mb-0.5">Taker</div>
                <div className="relative h-1.5 rounded-full overflow-hidden" style={{ background: 'rgba(255,255,255,0.06)' }}>
                  <div className="absolute inset-y-0 left-0 rounded-full" style={{
                    width: `${(comp.taker / maxFee) * 100}%`,
  ...
  return (
    <div>
      <div className="flex items-center gap-4 mb-3">
        {[
          { label: 'Spot', color: '#22c55e' },
          { label: 'Auction', color: '#a855f7' },
      ...
      <svg viewBox={`0 0 100 ${chartH + 18}`} className="w-full h-48" preserveAspectRatio="none">
        {VOLUME_DATA.map((d, i) => {
          ...
          const w = barW * 0.7
          const totalH = ((d.spot + d.auction + d.bridge) / maxVal) * chartH
          const spotH = (d.spot / maxVal) * chartH
          const auctionH = (d.auction / maxVal) * chartH
          const bridgeH = (d.bridge / maxVal) * chartH
          return (
            <g key={d.month}>
              <rect x={x} y={chartH - totalH} width={w} height={bridgeH} rx={0.5} fill="#3b82f6" opacity={0.8} />
              <rect x={x} y={chartH - totalH + bridgeH} width={w} height={auctionH} rx={0} fill="#a855f7" opacity={0.8} />
              <rect x={x} y={chartH - totalH + bridgeH + auctionH} width={w} height={spotH} rx={0} fill="#22c55e" opacity={0.8} />
              <text x={x + w / 2} y={chartH + 10} textAnchor="middle" fill="#737373" fontSize="3" fontFamily="monospace">
                {d.month}
              </text>
            </g>
          )
  ...
  return (
    <div className="space-y-1.5">
      <div className="grid grid-cols-5 gap-2 px-3 mb-2">
        <div className="text-[9px] font-mono text-black-600 uppercase tracking-wider col-span-1">Chain</div>
        <div className="text-[9px] font-mono text-black-600 uppercase tracking-wider text-right">Bridge Fee</div>
        <div className="text-[9px] font-mono text-black-600 uppercase tracking-wider text-right">Gas (est.)</div>
        <div className="text-[9px] font-mono text-black-600 uppercase tracking-wider text-right">Time</div>
        <div className="text-[9px] font-mono text-black-600 uppercase tracking-wider text-right">Protocol</div>
      </div>
      {BRIDGE_FEES.map((b, i) => (
        <motion.div key={b.chain} custom={i} variants={cardV} initial="hidden" animate="visible"
          ...
          <div className="text-right">
            <span className="text-xs font-mono text-black-300">{b.gasFee}</span>
          </div>
          <div className="text-right">
            <span className="text-[10px] font-mono text-black-400">{b.time}</span>
          </div>
          <div className="text-right">
            <span className="text-[9px] font-mono" style={{ color: CYAN }}>LayerZero</span>
          </div>
        </motion.div>
      ))}
  ...
  return (
    <div className="flex flex-col sm:flex-row items-center gap-6">
      <svg width={size} height={size} viewBox={`0 0 ${size} ${size}`} className="flex-shrink-0">
        ...
        <text x={cx} y={cy + 8} textAnchor="middle" fill="#737373" fontSize="5" fontFamily="monospace">
          Revenue
        </text>
      </svg>
...
function FeeSavingsCalculator() {
  ...
  const savings = useMemo(() => {
    const takerVolume = monthlyVolume * ((100 - makerRatio) / 100)
    const makerVolume = monthlyVolume * (makerRatio / 100)
    const tier = tierForVolume(monthlyVolume)
    const vs = FEE_TIERS[tier]

    const vibeMaker = makerVolume * (vs.makerFee / 100)
    const vibeTaker = takerVolume * (vs.takerFee / 100)
    const vibeTotal = vibeMaker + vibeTaker

    const uniTotal = monthlyVolume * 0.003
    const sushiTotal = monthlyVolume * 0.0025
    const dydxMaker = makerVolume * 0.0002
    const dydxTaker = takerVolume * 0.0005
    const dydxTotal = dydxMaker + dydxTaker

    return {
      tierName: vs.name,
      tierColor: vs.color,
      vibeTotal,
      vs_uniswap: uniTotal - vibeTotal,
      vs_sushiswap: sushiTotal - vibeTotal,
      vs_dydx: dydxTotal - vibeTotal,
      annual: (uniTotal - vibeTotal) * 12,
    }
      ...
      <div className="grid grid-cols-3 gap-2">
        {[
          ...
          <div key={c.label} className="rounded-xl p-3 text-center"
            style={{ background: c.val > 0 ? 'rgba(34,197,94,0.06)' : 'rgba(239,68,68,0.06)', border: `1px solid ${c.val > 0 ? 'rgba(34,197,94,0.15)' : 'rgba(239,68,68,0.15)'}` }}>
        ...
        <div className="max-w-5xl mx-auto px-4 space-y-6">
          ...
          {!isConnected && (
            <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }}
              transition={{ duration: 1 / PHI, delay: 1.2 }}
