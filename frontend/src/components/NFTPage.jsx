    ...
    <GlassCard glowColor="terminal" hover className="cursor-pointer" onClick={() => onClick?.(item)}>
      ...
      <div className={`h-40 bg-gradient-to-br ${item.gradient} relative overflow-hidden`}>
        ...
        <div
          ...
          style={{ color: r.color, backgroundColor: r.bg, border: `1px solid ${r.color}33` }}
        >
          {r.label}
        </div>
      </div>
...
export default function NFTPage() {
  ...
  const [auctionPhase, setAuctionPhase] = useState('commit') // commit | reveal | settled
  ...
  return (
    <div className="min-h-screen pb-8">
      <PageHero
        title="NFT Market"
        subtitle="Collect, trade, and earn soulbound achievements on the VibeSwap protocol"
        category="ecosystem"
        badge="Live"
        badgeColor={CYAN}
      />
              ...
              <GlassCard key={badge.id} glowColor={badge.earned ? 'terminal' : 'none'} className="p-4">
                <div className="flex items-start gap-3">
                  <div
                    ...
                    style={{
                      backgroundColor: badge.earned ? 'rgba(6,182,212,0.12)' : 'rgba(75,75,75,0.2)',
                      border: `1px solid ${badge.earned ? 'rgba(6,182,212,0.3)' : 'rgba(75,75,75,0.3)'}`,
                      color: badge.earned ? CYAN : '#6b7280',
                    }}
                  >
                    {badge.icon}
                  </div>
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center gap-2">
                      ...
                      {badge.earned && (
                        <span className="text-[10px] font-mono px-1.5 py-0.5 rounded" style={{ color: '#22c55e', backgroundColor: 'rgba(34,197,94,0.12)' }}>
                          Earned
                        </span>
                      )}
                    </div>
          ...
          <GlassCard glowColor="terminal" className="p-5">
            {!isConnected ? (
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
                ...
                <div>
                  <div
                    className="border-2 border-dashed border-gray-700 rounded-xl h-52 flex flex-col items-center justify-center cursor-pointer hover:border-gray-500 transition-colors"
                ...
                <div className="space-y-3">
                  <div>
                    ...
                    <input
                      type="text"
                      value={mintForm.name}
                      onChange={e => setMintForm(p => ({ ...p, name: e.target.value }))}
                      placeholder="My NFT"
                  ...
                  <div>
                    ...
                    <textarea
                      ...
                      rows={3}
                      className="w-full bg-black/30 border border-gray-700 rounded-lg px-3 py-2 text-sm text-white placeholder-gray-600 focus:outline-none focus:border-cyan-500/50 resize-none"
                    />
                  </div>
                  <div className="flex gap-3">
                    <div className="flex-1">
                      ...
                      <input
                        ...
                        onChange={e => setMintForm(p => ({ ...p, royalty: Number(e.target.value) }))}
                    ...
                    <div className="flex-1">
                      ...
                      <select
                        ...
                        onChange={e => setMintForm(p => ({ ...p, collection: e.target.value }))}
                        className="w-full bg-black/30 border border-gray-700 rounded-lg px-3 py-2 text-sm text-white focus:outline-none focus:border-cyan-500/50"
                      >
                        {COLLECTIONS.map(c => (
                  ...
                  <button
                    className="w-full py-2.5 rounded-lg text-sm font-bold font-mono transition-all"
                    style={{ backgroundColor: 'rgba(6,182,212,0.15)', color: CYAN, border: `1px solid rgba(6,182,212,0.3)` }}
                  >
                    Mint NFT
                  </button>
                </div>
              ...
              {['commit', 'reveal', 'settled'].map((phase, i) => (
                <div key={phase} className="flex items-center gap-2">
                  <div
                    ...
                    style={{
                      backgroundColor: auctionPhase === phase ? 'rgba(6,182,212,0.2)' : 'rgba(75,75,75,0.2)',
                      border: `1px solid ${auctionPhase === phase ? CYAN : 'rgba(75,75,75,0.4)'}`,
                      color: auctionPhase === phase ? CYAN : '#6b7280',
                    }}
                  >
                    {i + 1}
                  </div>
                  <span className={`text-xs font-mono capitalize ${auctionPhase === phase ? 'text-white' : 'text-gray-600'}`}>
                    {phase}
                  </span>
                </div>
              ))}
            ...
            <AnimatePresence mode="wait">
              {auctionPhase === 'commit' && (
                <motion.div key="commit" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="space-y-3">
                  <div className="flex gap-3">
                    <div className="flex-1">
                      ...
                      <input
                        type="text"
                        value={auctionCommit}
                        onChange={e => setAuctionCommit(e.target.value)}
                        placeholder="0.00"
                        className="w-full bg-black/30 border border-gray-700 rounded-lg px-3 py-2 text-sm text-white placeholder-gray-600 focus:outline-none focus:border-cyan-500/50"
                      />
                    </div>
                    <div className="flex-1">
                      ...
                      <input
                        type="text"
                        value={auctionSecret}
                        onChange={e => setAuctionSecret(e.target.value)}
                        placeholder="my-secret-123"
                        className="w-full bg-black/30 border border-gray-700 rounded-lg px-3 py-2 text-sm text-white placeholder-gray-600 focus:outline-none focus:border-cyan-500/50"
                      />
                    </div>
                  </div>
                  ...
                  <button
                    onClick={() => setAuctionPhase('reveal')}
                    className="w-full py-2.5 rounded-lg text-sm font-bold font-mono transition-all"
                    style={{ backgroundColor: 'rgba(6,182,212,0.15)', color: CYAN, border: `1px solid rgba(6,182,212,0.3)` }}
                  >
                    Submit Sealed Bid
              ...
              {auctionPhase === 'reveal' && (
                <motion.div key="reveal" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="space-y-3">
                  ...
                  <div className="flex items-center gap-2 text-xs font-mono text-gray-500">
                    <span>Committed:</span>
                    <span className="text-white">{auctionCommit || '0.00'} ETH</span>
                    <span className="text-gray-700">|</span>
                    <span>Secret:</span>
                    <span className="text-white">{auctionSecret || '(none)'}</span>
                  </div>
                  <button
                    onClick={() => setAuctionPhase('settled')}
                    className="w-full py-2.5 rounded-lg text-sm font-bold font-mono transition-all"
              ...
              {auctionPhase === 'settled' && (
                <motion.div key="settled" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="text-center py-6 space-y-2">
                  ...
                  <button
                    onClick={() => { setAuctionPhase('commit'); setAuctionCommit(''); setAuctionSecret('') }}
                    className="mt-3 px-4 py-1.5 rounded-lg text-xs font-mono text-gray-400 border border-gray-700 hover:text-white hover:border-gray-500 transition-all"
                  >
                    New Auction
                  </button>
                </motion.div>
              ...
              <table className="w-full text-left">
                <thead>
                  <tr className="border-b border-gray-800">
                    <th className="text-[10px] text-gray-500 font-mono uppercase tracking-wider px-4 py-3">Collection</th>
                    <th className="text-[10px] text-gray-500 font-mono uppercase tracking-wider px-4 py-3 text-right">Floor</th>
                    <th className="text-[10px] text-gray-500 font-mono uppercase tracking-wider px-4 py-3 text-right">24h Vol</th>
                    <th className="text-[10px] text-gray-500 font-mono uppercase tracking-wider px-4 py-3 text-right">Total Vol</th>
                    <th className="text-[10px] text-gray-500 font-mono uppercase tracking-wider px-4 py-3 text-right">Owners</th>
                    <th className="text-[10px] text-gray-500 font-mono uppercase tracking-wider px-4 py-3 text-right">Items</th>
                  </tr>
                </thead>
                <tbody>
                  {COLLECTIONS.map((c, i) => (
          ...
          {!isConnected ? (
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-4">
              ...
              <GlassCard glowColor="none" hover className="cursor-pointer border-dashed">
                <div className="h-32 flex items-center justify-center bg-black/20">
                  <span className="text-3xl text-gray-700">+</span>
                </div>
                <div className="p-3 text-center">
                  <div className="text-sm text-gray-500 font-mono">Mint New</div>
                </div>
              </GlassCard>
            </div>
          )}
            ...
            <div className="divide-y divide-gray-800/50">
              {ACTIVITY_FEED.map(event => (
                <div key={event.id} className="flex items-center gap-3 px-4 py-3 hover:bg-white/[0.02] transition-colors">
                  {/* Type badge */}
                  <div
                    className="w-16 text-center text-[10px] font-mono font-bold uppercase py-0.5 rounded"
                    style={{
                      color: event.type === 'sale' ? '#22c55e' : event.type === 'mint' ? '#a855f7' : '#f59e0b',
          ...
          <GlassCard glowColor="terminal" className="overflow-hidden">
            <div className="overflow-x-auto">
              <table className="w-full text-left">
                <thead>
                  <tr className="border-b border-gray-800">
                    <th className="text-[10px] text-gray-500 font-mono uppercase tracking-wider px-4 py-3">Trait</th>
                    <th className="text-[10px] text-gray-500 font-mono uppercase tracking-wider px-4 py-3">Value</th>
                    <th className="text-[10px] text-gray-500 font-mono uppercase tracking-wider px-4 py-3 text-right">Rarity</th>
                    <th className="text-[10px] text-gray-500 font-mono uppercase tracking-wider px-4 py-3 w-48">Distribution</th>
                  </tr>
                </thead>
                <tbody>
                  {RARITY_TRAITS.map(t => {
                      ...
                      <tr key={t.trait} className="border-b border-gray-800/50">
                        ...
                        <td className="px-4 py-3">
                          <div className="h-2 bg-gray-800 rounded-full overflow-hidden">
                            <motion.div
                              initial={{ width: 0 }}
                              animate={{ width: `${Math.min(t.rarity * (100 / 15), 100)}%` }}
                              transition={{ duration: 0.6, delay: 0.5, ease: [0.25, 0.1, 1 / PHI, 1] }}
