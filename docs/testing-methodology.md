...
| S33 | Solidity optimizer re-reads `block.timestamp` after `vm.warp` | Local variable `uint256 t0 = block.timestamp` gets optimized away — compiler substitutes fresh TIMESTAMP opcode. After `vm.warp(t0 + 50)`, `t0` reads the NEW warped value instead of the original. Debug showed `t0: 151` when it should be `1`. Tests fail with wrong reward calculations because elapsed time is computed from wrong base. | **Never use `uint256 t0 = block.timestamp` + `vm.warp(t0 + X)` pattern. Always use absolute numeric timestamps: `vm.warp(1000); ... vm.warp(1050); ... vm.warp(1100);`. The Solidity optimizer treats `block.timestamp` as a pure opcode and may replace cached local variables with fresh reads. This only matters in Foundry tests where `vm.warp` changes the timestamp mid-execution.** | LiquidityGauge.t.sol |
| S33 | Mock strategy deposit() called transferFrom but vault uses safeTransfer | StrategyVault._deployToStrategy() does `IERC20.safeTransfer(strategy, idle)` then `IStrategy.deposit(idle)`. Mock strategy's deposit() tried to transferFrom(vault, strategy, amount) — double-moving tokens. ERC20InsufficientAllowance error because vault never approved strategy. | **When mocking IStrategy.deposit(), make it a no-op. The vault already transferred tokens via safeTransfer before calling deposit(). deposit() is just a notification, not a pull operation. Check the actual contract's deployment pattern before writing mocks.** | StrategyVaultFuzz.t.sol, StrategyVaultInvariant.t.sol |
