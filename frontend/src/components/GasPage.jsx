...
// cost estimator, and block analysis
// ============================================================
    ...
    <div className="grid grid-cols-4 gap-2">
      {GAS_LEVELS.map((g, i) => (
        <motion.button key={g.label} custom={i} variants={cardV} initial="hidden" animate="visible"
          ...
          <div className="text-[9px] font-mono text-black-500 uppercase tracking-wider">{g.label}</div>
      ...
      <div className="flex items-center justify-center gap-4 mt-2">
        {[
          ...
          <div key={l.label} className="flex items-center gap-1.5">
            ...
            <span className="text-[9px] font-mono text-black-500">{l.label}</span>
          </div>
        ))}
      </div>
    </div>
...
function CostEstimator() {
  ...
  return (
    <div className="space-y-1.5">
          ...
          <motion.div key={tx.type} custom={i} variants={cardV} initial="hidden" animate="visible"
            ...
            <div className="flex-1 min-w-0">
              ...
              <div className="flex items-center gap-3 mt-0.5">
                <span className="text-[9px] font-mono text-black-500">{tx.gas.toLocaleString()} gas</span>
                <span className="text-[9px] font-mono text-black-600">{tx.desc}</span>
              </div>
            </div>
      ...
      <div className="text-[9px] font-mono text-black-600 text-center mt-2">
  ...
  return (
    <div className="min-h-screen pb-20">
      ...
      <div className="relative z-10">
        <PageHero title="Gas Tracker" category="system" subtitle="Real-time gas prices and cost estimation"
          badge={`${Math.round(currentGas)} gwei`} badgeColor={currentGas > 40 ? '#ef4444' : currentGas > 25 ? '#f59e0b' : '#22c55e'} />
              ...
              {CHAIN_GAS.map((c, i) => (
                <motion.div key={c.chain} custom={i} variants={cardV} initial="hidden" animate="visible"
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <div className="flex items-center gap-3 mt-0.5">
                      <span className="text-[10px] font-mono text-black-400">{c.gas} {c.unit}</span>
                      <span className="text-[9px] font-mono text-black-600">Block time: {c.speed}</span>
                    </div>
                  </div>
                  <div className="text-right flex-shrink-0">
                    <div className="text-sm font-mono font-bold" style={{ color: c.color }}>{c.txCost}</div>
                    <div className="text-[9px] font-mono text-black-500">per transfer</div>
                  </div>
                </motion.div>
              ))}
            </div>
                  ...
                  <motion.div key={b.number} custom={i} variants={cardV} initial="hidden" animate="visible"
                    ...
                    <div className="flex items-center justify-between mb-2">
                      <div className="flex items-center gap-2">
                        ...
                        <span className="text-[9px] font-mono text-black-500">{b.txCount} txns</span>
                      </div>
                      <div className="flex items-center gap-3">
                        <span className="text-[10px] font-mono text-black-400">{b.baseFee} gwei</span>
                        <span className="text-[9px] font-mono text-black-600">{b.time}</span>
                      </div>
                    </div>
            ...
            <div className="space-y-2">
              {[
                ...
                <motion.div key={t.tip} custom={i} variants={cardV} initial="hidden" animate="visible"
                  ...
                  <p className="text-[10px] font-mono text-black-400 mt-0.5">{t.desc}</p>
                </motion.div>
              ))}
            </div>
          </Section>
        ...
        <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 1.5 }} className="mt-12 mb-8 text-center">
          ...
          <p className="text-[10px] font-mono text-black-600 tracking-widest uppercase">Gas Tracker — EIP-1559</p>
        </motion.div>
      </div>
    </div>
  )
