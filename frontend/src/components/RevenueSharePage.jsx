...
const DISTRIBUTION_HISTORY = (() => {
  const baseEpoch = 1847
  return Array.from({ length: 8 }, (_, i) => {
    const totalDist = 20_000 + Math.floor(rand() * 10_000)
    return {
      epoch: baseEpoch - i,
      date: Date.now() - i * 86400000,
      totalDistributed: totalDist,
      perStakerShare: totalDist * 0.40 / 8_200,
      perLpShare: totalDist * 0.30 / 5_400,
      treasuryAllocation: totalDist * 0.20,
      buybackBurn: totalDist * 0.10,
    }
  })
})()
...
// ============ Revenue Flow Node ============
function FlowNode({ label, icon, color, pct, delay }) {
  return (
    <motion.div
    ...
    >
      <div
        className="w-14 h-14 rounded-xl flex items-center justify-center text-xl font-bold mb-1.5"
        style={{ background: `${color}15`, border: `1px solid ${color}30`, color }}
      >
        {icon}
      </div>
      <div className="text-xs font-mono font-bold text-white text-center leading-tight">{label}</div>
      {pct !== undefined && (
        <div className="text-[10px] font-mono mt-0.5" style={{ color }}>{pct}%</div>
      )}
    </motion.div>
  )
}
...
// ============ Flow Arrow ============
function FlowArrow({ delay, color = CYAN }) {
  return (
    <motion.div
    ...
    >
      <div className="relative h-0.5 w-8 sm:w-12" style={{ background: `${color}40` }}>
        <motion.div
...
export default function RevenueSharePage() {
  ...
  const projections = useMemo(() => {
    ...
    const dailyUsd = dailyJul * REVENUE_STATS.julPrice
    const monthlyUsd = monthlyJul * REVENUE_STATS.julPrice
    const yearlyUsd = yearlyJul * REVENUE_STATS.julPrice

    const apr = totalStaked > 0
      ? ((REVENUE_STATS.daily * 365 * 0.40) / totalStaked) * 100
      : 0
  ...
  if (!isConnected) {
    return (
      <div className="max-w-4xl mx-auto px-4 py-8">
        <PageHero
          title="Revenue Share"
          subtitle="Protocol fees flow back to the community — cooperative capitalism in action"
          category="defi"
        />
    ...
    <div className="max-w-5xl mx-auto px-4 py-6 space-y-8">
      ...
      <Section num="01" title="Revenue Flow" delay={0.05}>
        <GlassCard glowColor="terminal" className="p-6">
          ...
          <div className="hidden sm:flex items-center justify-center gap-2">
            <FlowNode label="Fee Collection" icon="$" color={CYAN} delay={0.08} />
            <FlowArrow delay={0.14} />
            <FlowNode label="Treasury" icon="\u2B23" color="#a78bfa" delay={0.20} />
            <FlowArrow delay={0.26} color="#a78bfa" />
              ...
              {DISTRIBUTION_SPLITS.map((split, i) => (
                ...
                >
                  <div
                    ...
                    style={{ background: `${split.color}15`, border: `1px solid ${split.color}30`, color: split.color }}
                  >
                    {split.icon}
                  </div>
                  <div>
                    <div className="text-xs font-mono font-bold text-white">{split.label}</div>
                    <div className="text-[10px] font-mono" style={{ color: split.color }}>{split.pct}%</div>
                  </div>
                  <div className="h-1.5 flex-1 min-w-[60px] rounded-full overflow-hidden" style={{ background: '#1f2937' }}>
                    <motion.div
                      className="h-full rounded-full"
                      style={{ background: split.color }}
                      initial={{ width: 0 }}
                      animate={{ width: `${split.pct * 2.5}%` }}
          ...
          <div className="sm:hidden space-y-3">
            <div className="flex items-center justify-center gap-3 mb-4">
              <FlowNode label="Fees" icon="$" color={CYAN} delay={0.08} />
              <FlowArrow delay={0.14} />
              <FlowNode label="Treasury" icon="\u2B23" color="#a78bfa" delay={0.20} />
            </div>
            <div className="grid grid-cols-2 gap-2">
              {DISTRIBUTION_SPLITS.map((split, i) => (
                <motion.div
                  ...
                  style={{ background: `${split.color}08`, borderColor: `${split.color}20` }}
                >
                  <div className="text-lg" style={{ color: split.color }}>{split.icon}</div>
                  <div className="text-[10px] font-mono font-bold text-white mt-1">{split.label}</div>
                  <div className="text-sm font-mono font-bold mt-0.5" style={{ color: split.color }}>{split.pct}%</div>
                </motion.div>
              ))}
            </div>
          </div>
          ...
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
            {FEE_SOURCES.map((source, i) => (
              <motion.div
                ...
                style={{ background: 'rgba(0,0,0,0.2)', borderColor: '#1f2937' }}
              >
                <div className="font-mono text-base font-bold" style={{ color: source.color }}>
                  {fmt(source.daily)}
                </div>
        ...
        <GlassCard glowColor="terminal" className="overflow-hidden">
          ...
          {pagedHistory.map((row, i) => (
            ...
            >
              ...
              <div className="font-mono text-sm" style={{ color: '#06b6d4' }}>{row.perStakerShare.toFixed(2)} JUL</div>
      ...
      <Section num="05" title="Claim Rewards" delay={0.34}>
        <GlassCard glowColor="terminal" className="p-5">
            ...
            <div>
              ...
              <div className="flex items-baseline gap-2">
                <span className="text-3xl font-mono font-bold" style={{ color: '#34d399' }}>
                  {fmt(YOUR_EARNINGS.pendingClaim)}
                </span>
          ...
          {/* Revenue source breakdown for claim */}
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
            {[
              ...
              <div
                ...
                style={{ background: 'rgba(0,0,0,0.2)', borderColor: '#1f2937' }}
              >
                <div className="font-mono text-sm font-bold" style={{ color: source.color }}>
        ...
        <GlassCard glowColor="terminal" className="p-5">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-5 mb-5">
            <div>
              ...
              <div className="relative">
                <input
                  type="number"
                  value={calcStaked}
                  onChange={(e) => setCalcStaked(e.target.value)}
                  placeholder="0.00"
            ...
            <div>
              ...
              <div
                className="w-full rounded-xl px-4 py-3 font-mono text-lg font-bold flex items-center gap-2"
          ...
          <div className="grid grid-cols-3 gap-3">
            {[
              {
                label: 'Daily',
                jul: activeTab === 'staker' ? projections.dailyJul : projections.dailyJul * 0.75,
                usd: activeTab === 'staker' ? projections.dailyUsd : projections.dailyUsd * 0.75,
              },
              {
                label: 'Monthly',
                jul: activeTab === 'staker' ? projections.monthlyJul : projections.monthlyJul * 0.75,
                usd: activeTab === 'staker' ? projections.monthlyUsd : projections.monthlyUsd * 0.75,
              },
              {
                label: 'Yearly',
                jul: activeTab === 'staker' ? projections.yearlyJul : projections.yearlyJul * 0.75,
          ...
          <div className="mt-4 p-3 rounded-xl border" style={{ background: 'rgba(0,0,0,0.15)', borderColor: '#1f2937' }}>
            <div className="text-[10px] font-mono text-gray-500 leading-relaxed text-center">
              ...
              and total staked supply of {fmt(8_427_310)} JUL. Actual earnings vary with protocol volume and participation.
            </div>
          </div>
        </GlassCard>
      </Section>

      {/* ============ 07. Distribution Splits Visual ============ */}
      <Section num="07" title="How It Works" delay={0.46}>
        <GlassCard glowColor="terminal" className="p-5">
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
            {/* Left: Pie-like bar visualization */}
            <div>
              <div className="text-xs font-mono text-gray-400 mb-3">Revenue Split</div>
              <div className="space-y-3">
                {DISTRIBUTION_SPLITS.map((split, i) => (
                  <motion.div
                    ...
                    animate={{ opacity: 1, x: 0 }}
                    transition={{ delay: 0.48 + i * (0.05 * PHI) }}
                  >
                    <div className="flex items-center justify-between mb-1">
                      <div className="flex items-center gap-2">
                        <span style={{ color: split.color }}>{split.icon}</span>
                        <span className="text-xs font-mono text-white">{split.label}</span>
                      </div>
                      <span className="text-xs font-mono font-bold" style={{ color: split.color }}>{split.pct}%</span>
                    </div>
                    <div className="h-3 rounded-full overflow-hidden" style={{ background: '#1f2937' }}>
                      <motion.div
                        ...
                        transition={{ duration: PHI, delay: 0.50 + i * 0.1, ease: 'easeOut' }}
                      >
                        <motion.div
                    ...
                    <div className="text-[9px] font-mono text-gray-600 mt-0.5">
                      ~{fmt(REVENUE_STATS.daily * split.pct / 100)} JUL daily
                    </div>
                  </motion.div>
                ))}
              </div>
            ...
            <div className="space-y-3">
              <div className="text-xs font-mono text-gray-400 mb-1">Distribution Mechanics</div>
              {[
                {
                  title: 'Batch Settlement',
                  desc: 'Revenue is collected per 10-second batch cycle. Commit-reveal auctions ensure MEV-free fee collection.',
                  icon: '\u25C6',
                },
                {
                  title: 'Epoch Distribution',
                  desc: 'Accumulated fees are distributed every epoch (~24h). Shapley values weight contributions fairly.',
                  icon: '\u25C7',
                },
                {
                  title: 'Cooperative Model',
                  desc: 'No extractive fees. 100% of protocol revenue flows to stakers, LPs, treasury, and buyback. Zero team allocation.',
                  icon: '\u25CB',
                },
                {
                  title: 'Auto-Compound',
                  desc: 'Enable auto-compound to restake your revenue share automatically. Compound interest accelerates returns.',
                  icon: '\u25CE',
                },
              ].map((item, i) => (
                <motion.div
                ...
                >
                  <div className="flex items-start gap-2">
                    <span className="text-sm shrink-0" style={{ color: CYAN }}>{item.icon}</span>
                    <div>
                      <div className="text-xs font-mono font-bold text-white">{item.title}</div>
                      <div className="text-[10px] font-mono text-gray-500 mt-0.5 leading-relaxed">{item.desc}</div>
                    </div>
                  </div>
                </motion.div>
              ))}
