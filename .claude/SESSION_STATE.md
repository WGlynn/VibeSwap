...
6. ✓ **Corpus repair** — `feedback_no-blockquotes-on-copy-paste-drafts.md` YAML frontmatter had an unquoted embedded `---` from a paste accident. Fixed in both local memory + substrate mirror. Commits `38a8f94` (jarvis-monorepo) + `674bd94` (memory). **REGRESSION SURFACED 20:30 by substrate-sync**: between Edit (20:21) and commit (20:24) some upstream process re-injected `originSessionId: <current-session>` into the description field, so commit `674bd94` shipped the re-corrupted file and the sync pushed it to public substrate. `verify_primitive_corpus.py` now reports 457/458 on the substrate. Root cause unidentified — logged in `~/.claude/cron-prompts/_primitives-pending.md` with recommended triage order (instrument-diff-trace on next memory-file edit). Do NOT re-fix in isolation — fix the injector.
7. ✓ **VibeSwap README rewrite** for honest external onramp (commit `cb3ee3e6` on WGlynn/VibeSwap, pushed). Same playbook as the JARVIS rewrite, driven by a second external-LLM eval roleplay. Status callout up top (not publicly deployed, CKB dev chain mining but cells spec-only), multi-audience entry table (auditor / mechanism-cribber / CKB engineer / researcher / trader — each with read-this/skip-that), core-vs-extension scope delineation (`contracts/core/` audit-scope vs 24 sibling module dirs as research surface), CKB pivot section added, counts corrected against deployed state (431 sol / 579 tests / 34 modules / 26 CKB cells / 123 papers / 0 deploys — old README claimed 376/510/32/138), Tony-Stark-cave epigraph + "built from scratch" paragraph demoted from top to About section near bottom.
...
4. **Desktop cleanup**:
   ...
   - Moved 15 loose top-level files ([REDACTED-NDA] compiled/drafts PDFs, USD8 partnership PDFs from Apr 27-29, Shapley/compression onepagers, rickaudio recordings, HUDDLE-TODAY.md) into the archive subfolder. Fully reversible.
...
1. **[REDACTED-NDA] compiled response** — sent 5-item drafts compiled into single-message PDF on Desktop. JARVIS-isn't-a-chat-wrapper pushback in item 4. F·draft-[REDACTED-NDA]-replies-on-behalf saved.
...
8. **`P·scope-drift-to-recent` saved** — Will caught the [REDACTED-NDA] weekly summary scope failure (defaulted to chat-context instead of file-system substrate).
...
10. **[REDACTED-NDA] daily report `2026-04-29_daily.md` + corrected weekly summary `2026-04-29_drafts-compiled.md`** with file-system + git-log scope (correcting the earlier session-only summary).
...
- ~10 new memory primitives: P·handshake-math-claim-determinism, P·complementary-lenses-audit-vs-mechanism-design, P·scope-drift-to-recent, P·dont-make-will-look-dumb (parent), F·persist-partner-architecture-aggressively, F·draft-[REDACTED-NDA]-replies-on-behalf, F·two-gate-types-framing-vs-substance, F·partner-facing-additive-framing, F·dont-auto-demote-on-alternative, F·usd8-non-extractive-not-yet-earned, U·knowledge-gaps (audit-lens cultivation goal added), J·usd8-architecture (canonical 9-layer)
...
- [REDACTED-NDA] compiled response (`Desktop/2026-04-29_[REDACTED-NDA]-compiled.pdf`) ready to send
...
After Oracle C12 shipped, Will asked for an E2E backend based on the [REDACTED-NDA]-intake A-3 product ("persistence layer for the how of professional knowledge work"). Stood up at `C:/Users/Will/lineage/` — FastAPI + SQLModel + SQLite, runs without infrastructure.
...
**PDF on Desktop**: `2026-04-18_Code_As_Coordination_v2.pdf` (70KB) — the thesis Will will forward to [REDACTED-NDA] for the Gemini-vs-Claude comparison.
...
### Append C24 to today's [REDACTED-NDA] report
