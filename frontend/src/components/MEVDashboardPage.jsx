...
const PROTOCOL_STATS = {
  totalTradesProtected: 847_293,
  estimatedMEVPrevented: 14_720_000,
  uniqueUsersProtected: 38_412,
  avgSavingsPerTrade: 17.37,
  totalSavings: 14_720_000,
  batchesCompleted: 312_847,
}
...
const ACTIVITY_FEED = Array.from({ length: 8 }, (_, i) => {
  ...
  const attackIdx = Math.floor(r() * 3)
  const tokenIdx = Math.floor(r() * TOKENS.length)
  const estimatedLoss = +(r() * 80 + 5).toFixed(2)
  const minsAgo = Math.floor(r() * 120) + 1
  return {
    id: i,
    type: ATTACK_LABELS[attackIdx],
    color: ATTACK_COLORS[ATTACK_LABELS[attackIdx]],
    token: TOKENS[tokenIdx],
    pair: `${TOKENS[tokenIdx]}/USDC`,
    estimatedLoss,
    actualResult: '$0.00',
    savings: estimatedLoss,
          ...
          <div className="flex items-center gap-1">
            {attack.diagram.map((step, j) => (
              <div key={j} className="flex items-center gap-1 flex-1">
                <div
                  className="flex-1 text-center py-1.5 px-2 rounded text-[11px] font-mono"
                  style={{ backgroundColor: `${attack.color}12`, color: attack.color, border: `1px solid ${attack.color}25` }}
                >
                  {step}
                </div>
                {j < attack.diagram.length - 1 && (
        ...
        <div className="space-y-3">
          <div>
            <div className="flex items-center justify-between mb-1">
              <span className="text-xs text-zinc-400">Traditional DEX</span>
              <span className="text-xs font-mono text-red-400">83% exposed</span>
            </div>
          ...
          <div>
            <div className="flex items-center justify-between mb-1">
              <span className="text-xs text-zinc-400">VibeSwap</span>
              <span className="text-xs font-mono text-green-400">0% exposed</span>
            </div>
            <div className="h-3 rounded-full bg-zinc-800 overflow-hidden">
              <motion.div
                className="h-full rounded-full"
          ...
          >
            <div className="flex items-center justify-center gap-2">
              <div
                ...
                style={{
                  backgroundColor: activeStep === i ? `${step.color}25` : 'transparent',
                  color: activeStep === i ? step.color : 'inherit',
                  border: `1.5px solid ${activeStep === i ? step.color : 'rgba(63,63,70,0.5)'}`,
                }}
              >
                {step.step}
              </div>
              ...
              return (
                <div>
                  ...
                  <div className="p-3 rounded-lg bg-zinc-900 border border-zinc-800">
                    ...
                    <code className="text-xs font-mono break-all" style={{ color: step.color }}>
                      {step.detail}
                    </code>
                  </div>
  ...
  return (
    <div className="min-h-screen pb-20">
      ...
      <div className="max-w-7xl mx-auto px-4 space-y-10">
        ...
        <section>
          <SectionHeader
            title="Attack Types Explained"
            subtitle="MEV bots extract value from traders in three primary ways. VibeSwap eliminates all of them."
            index={1}
          />
        ...
        {isConnected && (
          ...
          >
            <SectionHeader
              title="Your MEV Savings"
              subtitle="Personal protection statistics from your commit-reveal trades"
              index={2}
            />
            <UserSavings />
          </motion.section>
          ...
          <motion.section custom={2} variants={sectionV} initial="hidden" animate="visible">
            <GlassCard className="p-6 text-center">
              ...
              <div className="text-xs text-zinc-600 font-mono">
                Your trades are always protected, even without connecting
              </div>
            </GlassCard>
          </motion.section>
        )}
        ...
        <section>
          <SectionHeader
            title="Protocol-Wide Protection"
            subtitle="Aggregate MEV prevention across all VibeSwap users and chains"
            index={3}
          />
          <ProtocolStats />
        </section>
        ...
        <section>
          <SectionHeader
            title="MEV Activity Feed"
            subtitle="Recent prevented attacks in real-time"
            index={4}
          />
        ...
        <section>
          <SectionHeader
            title="VibeSwap vs Traditional DEX"
            subtitle="Side-by-side MEV exposure comparison"
            index={5}
          />
          <ComparisonChart />
        </section>
        ...
        <section>
          <SectionHeader
            title="How Commit-Reveal Eliminates MEV"
            subtitle="Three steps that make value extraction structurally impossible"
            index={6}
          />
          <HowItWorksSection />
        </section>
