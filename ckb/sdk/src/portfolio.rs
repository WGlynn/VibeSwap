...
mod tests {
    ...
    fn test_lp_half_pool_share() {
        ...
        let pos = &portfolio.lp_positions[0];
        // 50% of 100 ETH = 50 ETH @ $3000 = $150,000
        assert!(pos.token0_value > 149_000 * PRECISION);
        assert!(pos.token0_value < 151_000 * PRECISION);
        // 50% of 300K USDC = $150,000
        assert!(pos.token1_value > 149_000 * PRECISION);
        assert!(pos.token1_value < 151_000 * PRECISION);
    }

    #[test]
    fn test_il_3x_price_move() {
        // 3x price move: significant IL
        let pool = PoolCellData {
            reserve0: 100 * PRECISION,
            reserve1: 300 * PRECISION, // current price = 3
            ..make_pool(1, 2, 100 * PRECISION, 300 * PRECISION)
        };
        let entry_price = PRECISION; // was 1:1, now 3:1

        let il = calculate_il(entry_price, &pool, 0, 0);
        assert!(il > 0, "3x move should produce positive IL");
        assert!(il <= 10_000, "IL capped at 10000 bps");
    }

    #[test]
    fn test_il_small_price_change_near_zero_il() {
        // 1% price change should produce very small IL
        let pool = PoolCellData {
            reserve0: 100 * PRECISION,
            reserve1: 101 * PRECISION, // 1% above entry
            ..make_pool(1, 2, 100 * PRECISION, 101 * PRECISION)
        };
        let entry_price = PRECISION;

        let il = calculate_il(entry_price, &pool, 0, 0);
        // IL for 1% change is negligible (about 0.0025%)
        assert!(il < 10, "1% price move should produce < 10 bps IL, got {}", il);
    }

    #[test]
    fn test_lending_health_factor_at_unity() {
        // Health factor at exactly 1.0 (precision)
        // HF = (collateral * price * liq_threshold) / (debt * debt_price)
        // With default liq_threshold = 0.8, need col_value * 0.8 = debt_value
        // 12.5 ETH * $3000 * 0.8 = $30K collateral adjusted, 30K debt → HF = 1.0
        // But default liq_threshold is DEFAULT_LIQUIDATION_THRESHOLD, let's verify
        let prices = make_prices();
        let pool = make_lending_pool();
        // col_value * liq_threshold / debt_value = HF
        // We want HF = PRECISION exactly
        // collateral = X ETH, price = 3000, liq_threshold = pool.liquidation_threshold
        // debt = Y USDC, price = 1
        // HF = (X * 3000 * liq_threshold) / Y
        // Set X = 10, then (10 * 3000 * liq_threshold) / Y = PRECISION
        // Y = 10 * 3000 * liq_threshold / PRECISION = 30000 * liq_threshold / PRECISION
        let liq_t = pool.liquidation_threshold;
        let target_debt = vibeswap_math::mul_div(30_000 * PRECISION, liq_t, PRECISION);
        let vault = make_vault(10 * PRECISION, target_debt);
        ...
        let pos = &portfolio.lending_positions[0];
        // HF should be very close to PRECISION (1.0)
        let diff = if pos.health_factor > PRECISION {
            pos.health_factor - PRECISION
        } else {
            PRECISION - pos.health_factor
        };
        assert!(diff < PRECISION / 100, "HF should be ~1.0, got diff {}", diff);
    }

    #[test]
    fn test_lending_collateral_amount_in_output() {
        let prices = make_prices();
        let vault = make_vault(42 * PRECISION, 5_000 * PRECISION);
        let pool = make_lending_pool();
        ...
        assert_eq!(portfolio.lending_positions[0].collateral_amount, 42 * PRECISION);
    }

    #[test]
    fn test_insurance_very_high_premium_yield() {
        // premiums = 50K, claims = 0 on 100K deposits = 50% yield = 5000 bps
        let ins = InsurancePoolCellData {
            total_premiums_earned: 50_000 * PRECISION,
            total_claims_paid: 0,
            ..make_insurance()
        };
        ...
        assert_eq!(portfolio.insurance_positions[0].premium_yield_bps, 5_000);
    }

    #[test]
    fn test_prediction_all_tiers_zero() {
        // All tier pools are 0 → payout = 0
        let market = PredictionMarketCellData {
            market_id: [0xAB; 32],
            num_tiers: 3,
            tier_pools: [0u128; 8],
    ...
    #[test]
    fn test_token_large_quantity() {
        // Very large token amount should not overflow
        let mut builder = PortfolioBuilder::new([0x11; 32], make_prices());
        builder.add_token(token(2), u128::MAX / PRECISION); // Many USDC at $1
        let portfolio = builder.build();

        assert!(portfolio.token_balances[0].value > 0);
    }

    #[test]
    fn test_empty_portfolio_has_empty_positions() {
        let builder = PortfolioBuilder::new([0x11; 32], make_prices());
        let portfolio = builder.build();
    ...
    #[test]
    fn test_lending_fractional_borrow_index() {
        // Borrow index at 1.5x → debt = 1.5 * original
        let prices = make_prices();
        let vault = VaultCellData {
            borrow_index_snapshot: PRECISION,
            debt_shares: 10_000 * PRECISION,
        ...
        let pool = LendingPoolCellData {
            borrow_index: PRECISION + PRECISION / 2, // 1.5x
            ..make_lending_pool()
        };
    ...
    #[test]
    fn test_multiple_lending_positions_total_debt() {
        let prices = make_prices();
        let pool = make_lending_pool();

        let mut builder = PortfolioBuilder::new([0x11; 32], prices);
        builder.add_vault(make_vault(10 * PRECISION, 5_000 * PRECISION), pool.clone());
        builder.add_vault(make_vault(5 * PRECISION, 3_000 * PRECISION), pool.clone());
        builder.add_vault(make_vault(20 * PRECISION, 12_000 * PRECISION), pool);
        let portfolio = builder.build();

        assert_eq!(portfolio.lending_positions.len(), 3);
        assert_eq!(portfolio.summary.total_debt, 20_000 * PRECISION);
        assert_eq!(portfolio.summary.total_collateral, (10 + 5 + 20) * 3_000 * PRECISION);
    }

    #[test]
    fn test_prediction_market_id_and_tier_preserved() {
        let market = PredictionMarketCellData {
            market_id: [0xBB; 32],
            num_tiers: 2,
            tier_pools: {
        ...
        assert_eq!(portfolio.prediction_positions[0].market_id, [0xBB; 32]);
        assert_eq!(portfolio.prediction_positions[0].tier_index, 0);
        assert_eq!(portfolio.prediction_positions[0].amount, 1_000 * PRECISION);
    }

    #[test]
    fn test_insurance_high_price_token() {
        // Insurance pool in BTC ($60K)
        let ins = InsurancePoolCellData {
            asset_type_hash: token(3), // BTC
            pool_id: [0x55; 32],
            total_deposits: 100 * PRECISION,
            total_shares: 100 * PRECISION,
            total_premiums_earned: 5 * PRECISION,
            total_claims_paid: 0,
            ..make_insurance()
        };

        let mut builder = PortfolioBuilder::new([0x11; 32], make_prices());
        builder.add_insurance(ins, 10 * PRECISION); // 10% of pool
        let portfolio = builder.build();

        // 10/100 * (100 + 5 - 0) = 10.5 BTC @ $60K = $630K
        let pos = &portfolio.insurance_positions[0];
        assert_eq!(pos.underlying_value, 630_000 * PRECISION);
    }

    #[test]
    fn test_portfolio_only_tokens_protocol_count_one() {
        let mut builder = PortfolioBuilder::new([0x11; 32], make_prices());
        builder.add_token(token(1), PRECISION);
        let portfolio = builder.build();
    ...
    #[test]
    fn test_portfolio_only_lending_protocol_count_one() {
        let mut builder = PortfolioBuilder::new([0x11; 32], make_prices());
        builder.add_vault(make_vault(10 * PRECISION, 5_000 * PRECISION), make_lending_pool());
        let portfolio = builder.build();
    ...
    #[test]
    fn test_lp_with_asymmetric_reserves() {
        // Very asymmetric pool: 1 ETH and 10M USDC
        let prices = make_prices();
        let pool = make_pool(1, 2, PRECISION, 10_000_000 * PRECISION);
        let lp = make_lp(pool.total_lp_supply / 10, PRECISION);
    ...
    #[test]
    fn test_concentration_three_positions_one_dominant() {
        // 1 BTC ($60K), 1 ETH ($3K), 1000 USDC ($1K) → BTC dominates
        let mut builder = PortfolioBuilder::new([0x11; 32], make_prices());
        builder.add_token(token(3), PRECISION);     // 1 BTC = $60K
        builder.add_token(token(1), PRECISION);     // 1 ETH = $3K
        builder.add_token(token(2), 1_000 * PRECISION); // 1K USDC = $1K
        let portfolio = builder.build();

        // Total = $64K, max = $60K → concentration = 60/64 * 10000 ≈ 9375 bps
        let total = 64_000u64;
        let expected_approx = 60_000u64 * 10_000 / total;
        let diff = if portfolio.summary.concentration_bps > expected_approx {
            portfolio.summary.concentration_bps - expected_approx
        } else {
            expected_approx - portfolio.summary.concentration_bps
        };
        assert!(diff < 100, "Concentration should be ~9375 bps, got {}", portfolio.summary.concentration_bps);
    }

    #[test]
    fn test_estimate_portfolio_apy_no_earning_positions() {
        // Only LP and tokens → no APY sources → 0
        let prices = make_prices();
        let pool = make_pool(1, 2, 100 * PRECISION, 300_000 * PRECISION);
        let lp = make_lp(pool.total_lp_supply / 10, 3_000 * PRECISION);
    ...
    #[test]
    fn test_lending_deposit_shares_in_output() {
        let prices = make_prices();
        let vault = VaultCellData {
            deposit_shares: 42_000 * PRECISION,
            ..make_vault(10 * PRECISION, 5_000 * PRECISION)
        };
        let pool = make_lending_pool();
        ...
        assert_eq!(portfolio.lending_positions[0].deposit_shares, 42_000 * PRECISION);
    }

    #[test]
    fn test_prediction_heavily_skewed_tier() {
        // One tier has 99% of pool → user in minority tier gets huge payout
        let market = PredictionMarketCellData {
            market_id: [0xCC; 32],
            num_tiers: 2,
            tier_pools: {
                let mut t = [0u128; 8];
                t[0] = 99_000 * PRECISION; // 99% of pool
                t[1] = 1_000 * PRECISION;  // 1% of pool
                t
            },
            status: vibeswap_types::MARKET_ACTIVE,
            ..Default::default()
        ...
        let position = PredictionPositionCellData {
            market_id: [0xCC; 32],
            tier_index: 1, // Bet on minority tier
        ...
        // payout = 500 * 100K / 1K = 50K (100x leverage)
        let expected = vibeswap_math::mul_div(
            500 * PRECISION,
            100_000 * PRECISION,
            1_000 * PRECISION,
    ...
    #[test]
    fn test_portfolio_build_multiple_times() {
        // Building from same builder multiple times should produce same result
        let prices = make_prices();
        let mut builder = PortfolioBuilder::new([0x11; 32], prices);
        builder.add_token(token(1), 10 * PRECISION);
        ...
        assert_eq!(p1.summary.total_value, p2.summary.total_value);
        assert_eq!(p1.token_balances.len(), p2.token_balances.len());
    }

    #[test]
    fn test_insurance_with_non_unit_price() {
        // Insurance pool denominated in ETH (price = $3000)
        let ins = InsurancePoolCellData {
            asset_type_hash: token(1), // ETH
            pool_id: [0x66; 32],
            total_deposits: 100 * PRECISION,
            total_shares: 100 * PRECISION,
            total_premiums_earned: 10 * PRECISION,
            total_claims_paid: 2 * PRECISION,
            ..make_insurance()
        };

        let mut builder = PortfolioBuilder::new([0x11; 32], make_prices());
        builder.add_insurance(ins, 50 * PRECISION); // 50% of pool
        let portfolio = builder.build();

        // Underlying = 50/100 * (100 + 10 - 2) = 54 ETH
        // Value = 54 ETH * $3000 = $162K
        assert_eq!(portfolio.insurance_positions[0].underlying_value, 162_000 * PRECISION);
    }

    #[test]
    fn test_lending_collateral_in_btc() {
        // Vault with BTC collateral
        let prices = make_prices();
        let vault = VaultCellData {
            collateral_type_hash: token(3), // BTC
            collateral_amount: PRECISION, // 1 BTC
            debt_shares: 30_000 * PRECISION,
            ..make_vault(PRECISION, 30_000 * PRECISION)
        };
        let pool = make_lending_pool();
        ...
        let pos = &portfolio.lending_positions[0];
        // 1 BTC @ $60K = $60K collateral
        assert_eq!(pos.collateral_value, 60_000 * PRECISION);
        assert_eq!(pos.debt_value, 30_000 * PRECISION);
        assert_eq!(pos.net_value, 30_000 * PRECISION);
    }

    // ============ Hardening Batch v4 ============

    #[test]
    fn test_lp_100_percent_of_pool_v4() {
        // User owns 100% of pool LP supply
        let prices = make_prices();
        let pool = make_pool(1, 2, 100 * PRECISION, 300_000 * PRECISION);
        let lp = make_lp(pool.total_lp_supply, 3_000 * PRECISION); // 100%
        ...
        let pos = &portfolio.lp_positions[0];
        // 100% of 100 ETH @ $3000 = $300,000
        assert_eq!(pos.token0_value, 300_000 * PRECISION);
        // 100% of 300K USDC @ $1 = $300,000
        assert_eq!(pos.token1_value, 300_000 * PRECISION);
    }

    #[test]
    fn test_lp_entry_price_preserved_v4() {
        let prices = make_prices();
        let pool = make_pool(1, 2, 100 * PRECISION, 300_000 * PRECISION);
        let lp = make_lp(pool.total_lp_supply / 10, 42_000 * PRECISION);

        let mut builder = PortfolioBuilder::new([0x11; 32], prices);
        builder.add_lp(lp, pool);
        let portfolio = builder.build();

        assert_eq!(portfolio.lp_positions[0].entry_price, 42_000 * PRECISION);
    }

    #[test]
    fn test_lp_amount_preserved_v4() {
        let prices = make_prices();
        let pool = make_pool(1, 2, 100 * PRECISION, 300_000 * PRECISION);
        let target_lp = pool.total_lp_supply / 5;
        let lp = make_lp(target_lp, 3_000 * PRECISION);

        let mut builder = PortfolioBuilder::new([0x11; 32], prices);
        builder.add_lp(lp, pool);
        let portfolio = builder.build();

        assert_eq!(portfolio.lp_positions[0].lp_amount, target_lp);
    }

    #[test]
    fn test_lending_deposit_shares_preserved_v4() {
        let prices = make_prices();
        let vault = VaultCellData {
            deposit_shares: 77_777 * PRECISION,
            ..make_vault(10 * PRECISION, 5_000 * PRECISION)
        };
        let pool = make_lending_pool();
        ...
        assert_eq!(portfolio.lending_positions[0].deposit_shares, 77_777 * PRECISION);
    }

    #[test]
    fn test_lending_collateral_amount_preserved_v4() {
        let prices = make_prices();
        let vault = make_vault(42 * PRECISION, 1_000 * PRECISION);
        let pool = make_lending_pool();
    ...
    #[test]
    fn test_insurance_shares_preserved_v4() {
        let ins = make_insurance();
        let mut builder = PortfolioBuilder::new([0x11; 32], make_prices());
        builder.add_insurance(ins, 12_345 * PRECISION);
        let portfolio = builder.build();

        assert_eq!(portfolio.insurance_positions[0].shares, 12_345 * PRECISION);
    }

    #[test]
    fn test_prediction_tier_index_preserved_v4() {
        let market = PredictionMarketCellData {
            market_id: [0xF0; 32],
            num_tiers: 4,
            tier_pools: {
    ...
    #[test]
    fn test_prediction_amount_preserved_v4() {
        let market = PredictionMarketCellData {
            market_id: [0xF1; 32],
            num_tiers: 2,
            tier_pools: {
        ...
        assert_eq!(portfolio.prediction_positions[0].amount, 99_999 * PRECISION);
    }

    #[test]
    fn test_prediction_market_id_preserved_v4() {
        let market = PredictionMarketCellData {
            market_id: [0xAB; 32],
            num_tiers: 2,
            tier_pools: {
    ...
    #[test]
    fn test_il_zero_reserve0_v4() {
        // reserve0 = 0 → IL = 0 (guard clause)
        let pool = PoolCellData {
            reserve0: 0,
            reserve1: 300_000 * PRECISION,
            ..make_pool(1, 2, 0, 300_000 * PRECISION)
        };
        let il = calculate_il(3_000 * PRECISION, &pool, 3_000 * PRECISION, PRECISION);
        assert_eq!(il, 0);
    }
    ...
    #[test]
    fn test_il_small_price_change_v4() {
        // 1.01x price move — very small IL
        let pool = PoolCellData {
            reserve0: 100 * PRECISION,
            reserve1: 101 * PRECISION,
            ..make_pool(1, 2, 100 * PRECISION, 101 * PRECISION)
        };
        let entry_price = PRECISION; // was 1:1
        let il = calculate_il(entry_price, &pool, 0, 0);
        // Very small price change → IL near 0
        assert!(il <= 10, "1% price move should give negligible IL, got {} bps", il);
    }

    #[test]
    fn test_insurance_full_shares_of_pool_v4() {
        // User owns 100% of insurance shares
        let ins = make_insurance();
        let mut builder = PortfolioBuilder::new([0x11; 32], make_prices());
        builder.add_insurance(ins.clone(), ins.total_shares);
        let portfolio = builder.build();

        // 100% of (100K + 5K - 1K) = 104K USDC @ $1 = $104K
        assert_eq!(portfolio.insurance_positions[0].underlying_value, 104_000 * PRECISION);
    }

    #[test]
    fn test_lending_very_high_borrow_index_v4() {
        // borrow_index = 10x snapshot → debt = 10x
        let prices = make_prices();
        let vault = make_vault(100 * PRECISION, 1_000 * PRECISION);
        let pool = LendingPoolCellData {
            borrow_index: 10 * PRECISION,
            ..make_lending_pool()
        };
    ...
    #[test]
    fn test_summary_only_tokens_protocol_count_v4() {
        let mut builder = PortfolioBuilder::new([0x11; 32], make_prices());
        builder.add_token(token(1), 10 * PRECISION);
        builder.add_token(token(2), 100 * PRECISION);
        let portfolio = builder.build();
    ...
    #[test]
    fn test_summary_only_lp_protocol_count_v4() {
        let prices = make_prices();
        let pool = make_pool(1, 2, 100 * PRECISION, 300_000 * PRECISION);
        let lp = make_lp(pool.total_lp_supply / 10, 3_000 * PRECISION);
    ...
    #[test]
    fn test_summary_only_insurance_protocol_count_v4() {
        let ins = make_insurance();
        let mut builder = PortfolioBuilder::new([0x11; 32], make_prices());
        builder.add_insurance(ins, 10_000 * PRECISION);
        let portfolio = builder.build();
    ...
    #[test]
    fn test_summary_only_lending_protocol_count_v4() {
        let prices = make_prices();
        let vault = make_vault(10 * PRECISION, 5_000 * PRECISION);
        let pool = make_lending_pool();
    ...
    #[test]
    fn test_lending_net_value_zero_when_debt_exceeds_all_v4() {
        // Debt > collateral + deposits → net_value = 0 (saturating)
        let prices = make_prices();
        let vault = VaultCellData {
            deposit_shares: 10_000 * PRECISION,
            ..make_vault(1 * PRECISION, 50_000 * PRECISION)
        };
        let pool = make_lending_pool();
        ...
        let pos = &portfolio.lending_positions[0];
        // collateral = 3K, deposit = 10K USDC = 10K, debt = 50K
        // net = 3K + 10K - 50K → saturating → 0
        assert_eq!(pos.net_value, 0);
    }

    #[test]
    fn test_concentration_zero_total_value_v4() {
        // All positions with zero value → concentration = 0
        let empty_prices = BTreeMap::new();
        let mut builder = PortfolioBuilder::new([0x11; 32], empty_prices);
        builder.add_token(token(99), 1_000 * PRECISION);
        let portfolio = builder.build();
    ...
    #[test]
    fn test_portfolio_owner_preserved_v4() {
        let owner = [0xDE; 32];
        let builder = PortfolioBuilder::new(owner, make_prices());
        let portfolio = builder.build();
        assert_eq!(portfolio.owner_lock_hash, owner);
    ...
    #[test]
    fn test_three_lp_positions_v4() {
        let prices = make_prices();
        let pool1 = make_pool(1, 2, 100 * PRECISION, 300_000 * PRECISION);
        let pool2 = make_pool(1, 3, 10 * PRECISION, 600_000 * PRECISION);
        let pool3 = make_pool(2, 3, 500_000 * PRECISION, 10 * PRECISION);
        let lp1 = make_lp(pool1.total_lp_supply / 10, 3_000 * PRECISION);
        let lp2 = make_lp(pool2.total_lp_supply / 20, 60_000 * PRECISION);
        let lp3 = make_lp(pool3.total_lp_supply / 50, PRECISION);
    ...
    #[test]
    fn test_lending_high_health_factor_v4() {
        // Very safe position: large collateral, tiny debt
        let prices = make_prices();
        let vault = make_vault(1000 * PRECISION, 100 * PRECISION); // 1000 ETH, 100 USDC debt
        let pool = make_lending_pool();
        ...
        let pos = &portfolio.lending_positions[0];
        // HF = (1000 ETH * $3000 * 0.8) / (100 * $1) = 24,000
        assert!(pos.health_factor > 1000 * PRECISION, "Very safe vault should have very high HF");
    }

    // ============ Hardening Round 5 Tests ============

    #[test]
    fn test_empty_portfolio_summary_all_zeroes_h5() {
        let prices = make_prices();
        let builder = PortfolioBuilder::new([0x11; 32], prices);
        let portfolio = builder.build();
        assert_eq!(portfolio.summary.total_value, 0);
        assert_eq!(portfolio.summary.lp_value, 0);
        assert_eq!(portfolio.summary.lending_net_value, 0);
        assert_eq!(portfolio.summary.insurance_value, 0);
        assert_eq!(portfolio.summary.prediction_value, 0);
        assert_eq!(portfolio.summary.token_value, 0);
        assert_eq!(portfolio.summary.protocol_count, 0);
    }
    ...
    #[test]
    fn test_lp_position_zero_reserves_h5() {
        let prices = make_prices();
        let pool = make_pool(1, 2, 0, 0); // Zero reserves
        let lp = make_lp(1000, PRECISION);
        let mut builder = PortfolioBuilder::new([0x11; 32], prices);
        builder.add_lp(lp, pool);
        let portfolio = builder.build();
        let pos = &portfolio.lp_positions[0];
        assert_eq!(pos.token0_value, 0);
        assert_eq!(pos.token1_value, 0);
    }

    #[test]
    fn test_lp_position_zero_lp_amount_h5() {
        let prices = make_prices();
        let pool = make_pool(1, 2, 1000 * PRECISION, 3_000_000 * PRECISION);
        let lp = make_lp(0, PRECISION); // Zero LP tokens
        let mut builder = PortfolioBuilder::new([0x11; 32], prices);
        builder.add_lp(lp, pool);
        let portfolio = builder.build();
        let pos = &portfolio.lp_positions[0];
    ...
    #[test]
    fn test_lending_position_no_debt_infinite_hf_h5() {
        let prices = make_prices();
        let mut vault = make_vault(100 * PRECISION, 0); // Zero debt
        vault.deposit_shares = 0;
        let pool = make_lending_pool();
        let mut builder = PortfolioBuilder::new([0x11; 32], prices);
        builder.add_vault(vault, pool);
        let portfolio = builder.build();
    ...
    #[test]
    fn test_lending_position_deposit_value_h5() {
        let prices = make_prices();
        let mut vault = make_vault(0, 0);
        vault.deposit_shares = 500_000 * PRECISION; // Half of total shares
        let pool = make_lending_pool();
        let mut builder = PortfolioBuilder::new([0x11; 32], prices);
        builder.add_vault(vault, pool);
        let portfolio = builder.build();
        let pos = &portfolio.lending_positions[0];
        // deposit_shares=500k, total_shares=1M, total_deposits=1M
        // underlying = 500k * 1M / 1M = 500k (in USDC at $1)
        assert!(pos.deposit_value > 0);
    }

    #[test]
    fn test_insurance_position_zero_shares_h5() {
        let prices = make_prices();
        let ins = make_insurance();
        let mut builder = PortfolioBuilder::new([0x11; 32], prices);
        builder.add_insurance(ins, 0); // Zero shares
        let portfolio = builder.build();
        let pos = &portfolio.insurance_positions[0];
        assert_eq!(pos.underlying_value, 0);
    }

    #[test]
    fn test_insurance_position_full_shares_h5() {
        let prices = make_prices();
        let ins = make_insurance();
        let total_shares = ins.total_shares;
        let mut builder = PortfolioBuilder::new([0x11; 32], prices);
        builder.add_insurance(ins, total_shares); // All shares
        let portfolio = builder.build();
        let pos = &portfolio.insurance_positions[0];
        assert!(pos.underlying_value > 0);
        assert_eq!(pos.shares, total_shares);
    }

    #[test]
    fn test_insurance_premium_yield_bps_h5() {
        let prices = make_prices();
        let ins = make_insurance();
        // premiums_earned=5000, claims_paid=1000, net=4000
        // yield = 4000 * 10000 / 100000 = 400 bps
        let mut builder = PortfolioBuilder::new([0x11; 32], prices);
        builder.add_insurance(ins, 50_000 * PRECISION);
        let portfolio = builder.build();
        let pos = &portfolio.insurance_positions[0];
        assert_eq!(pos.premium_yield_bps, 400);
    }

    #[test]
    fn test_prediction_position_settled_h5() {
        let prices = make_prices();
        let market = PredictionMarketCellData {
            market_id: [0x33; 32],
            status: vibeswap_types::MARKET_SETTLED,
        ...
        let pos_data = PredictionPositionCellData {
            market_id: [0x33; 32],
            tier_index: 0,
            amount: 500,
            owner_lock_hash: [0x11; 32],
        ...
        let mut builder = PortfolioBuilder::new([0x11; 32], prices);
        builder.add_prediction(market, pos_data);
        let portfolio = builder.build();
        let pos = &portfolio.prediction_positions[0];
        assert!(pos.is_settled);
    }
    ...
    #[test]
    fn test_prediction_position_not_settled_h5() {
        let prices = make_prices();
        let market = PredictionMarketCellData {
            market_id: [0x33; 32],
            status: 0, // Not settled
            num_tiers: 2,
            tier_pools: {
                let mut t = [0u128; 8];
                t[0] = 1000;
    ...
    #[test]
    fn test_token_balance_with_known_price_h5() {
        let prices = make_prices();
        let mut builder = PortfolioBuilder::new([0x11; 32], prices);
        builder.add_token(token(1), 10 * PRECISION); // 10 ETH
        let portfolio = builder.build();
        let tok = &portfolio.token_balances[0];
        // 10 ETH * $3000 = $30000
        assert_eq!(tok.value, 30_000 * PRECISION);
    }

    #[test]
    fn test_token_balance_unknown_price_zero_value_h5() {
        let prices = make_prices();
        let mut builder = PortfolioBuilder::new([0x11; 32], prices);
        builder.add_token(token(99), 1000 * PRECISION); // Unknown token
        let portfolio = builder.build();
        let tok = &portfolio.token_balances[0];
        assert_eq!(tok.value, 0); // price = 0 → value = 0
    }

    #[test]
    fn test_summary_protocol_count_all_five_h5() {
        let prices = make_prices();
        let pool = make_pool(1, 2, 1000 * PRECISION, 3_000_000 * PRECISION);
        let lp = make_lp(100 * PRECISION, PRECISION);
        let vault = make_vault(10 * PRECISION, 1000 * PRECISION);
        let lending_pool = make_lending_pool();
        let ins = make_insurance();
        let market = PredictionMarketCellData {
            market_id: [0x33; 32],
        ...
        let pred = PredictionPositionCellData {
            market_id: [0x33; 32],
            tier_index: 0,
            amount: 100,
            owner_lock_hash: [0x11; 32],
        ...
        let mut builder = PortfolioBuilder::new([0x11; 32], prices);
        builder.add_lp(lp, pool);
        builder.add_vault(vault, lending_pool);
        builder.add_insurance(ins, 1000 * PRECISION);
        builder.add_prediction(market, pred);
        builder.add_token(token(1), 5 * PRECISION);
        let portfolio = builder.build();
        assert_eq!(portfolio.summary.protocol_count, 5);
    }
    ...
    #[test]
    fn test_summary_worst_hf_no_lending_h5() {
        let prices = make_prices();
        let builder = PortfolioBuilder::new([0x11; 32], prices);
        let portfolio = builder.build();
        assert_eq!(portfolio.summary.worst_health_factor, u128::MAX);
    ...
    #[test]
    fn test_summary_concentration_single_token_h5() {
        let prices = make_prices();
        let mut builder = PortfolioBuilder::new([0x11; 32], prices);
        builder.add_token(token(1), 10 * PRECISION); // Only position
        let portfolio = builder.build();
        assert_eq!(portfolio.summary.concentration_bps, 10_000); // 100%
    }

    #[test]
    fn test_summary_concentration_two_equal_tokens_h5() {
        let prices = make_prices();
        let mut builder = PortfolioBuilder::new([0x11; 32], prices);
        builder.add_token(token(2), 1000 * PRECISION); // $1000
        builder.add_token(token(2), 1000 * PRECISION); // $1000
        let portfolio = builder.build();
        // Two equal positions → 50% each → concentration = 5000 bps
        assert_eq!(portfolio.summary.concentration_bps, 5000);
    }

    #[test]
    fn test_lp_il_same_price_zero_il_h5() {
        let prices = make_prices();
        // Set reserves to match entry price = PRECISION (1:1)
        let pool = make_pool(1, 2, 1000 * PRECISION, 1000 * PRECISION);
        let lp = make_lp(100 * PRECISION, PRECISION);
        let mut builder = PortfolioBuilder::new([0x11; 32], prices);
        builder.add_lp(lp, pool);
        let portfolio = builder.build();
        assert_eq!(portfolio.lp_positions[0].il_bps, 0);
    }

    #[test]
    fn test_portfolio_owner_hash_preserved_h5() {
        let prices = make_prices();
        let owner = [0xAB; 32];
        let builder = PortfolioBuilder::new(owner, prices);
        let portfolio = builder.build();
        assert_eq!(portfolio.owner_lock_hash, [0xAB; 32]);
    }

    #[test]
    fn test_multiple_lp_positions_count_h5() {
        let prices = make_prices();
        let pool1 = make_pool(1, 2, 1000 * PRECISION, 3_000_000 * PRECISION);
        let pool2 = make_pool(1, 2, 2000 * PRECISION, 6_000_000 * PRECISION);
        let lp1 = make_lp(100 * PRECISION, PRECISION);
        let lp2 = make_lp(200 * PRECISION, PRECISION);
        let mut builder = PortfolioBuilder::new([0x11; 32], prices);
        builder.add_lp(lp1, pool1).add_lp(lp2, pool2);
        let portfolio = builder.build();
        assert_eq!(portfolio.lp_positions.len(), 2);
    ...
    #[test]
    fn test_lending_borrow_index_accrual_2x_h5() {
        let prices = make_prices();
        let mut vault = make_vault(100 * PRECISION, 1000 * PRECISION);
        vault.borrow_index_snapshot = PRECISION;
        let mut pool = make_lending_pool();
        pool.borrow_index = 2 * PRECISION; // 2x accrual
        let mut builder = PortfolioBuilder::new([0x11; 32], prices);
        builder.add_vault(vault, pool);
        let portfolio = builder.build();
        let pos = &portfolio.lending_positions[0];
        // debt = 1000 * 2 = 2000 USDC
        assert_eq!(pos.debt_amount, 2000 * PRECISION);
    }

    #[test]
    fn test_lending_net_value_debt_exceeds_collateral_h5() {
        let prices = make_prices();
        // 1 ETH = $3000 collateral, 10000 USDC debt → net should saturate to 0
        let vault = make_vault(1 * PRECISION, 10_000 * PRECISION);
        let pool = make_lending_pool();
        let mut builder = PortfolioBuilder::new([0x11; 32], prices);
        builder.add_vault(vault, pool);
        let portfolio = builder.build();
        let pos = &portfolio.lending_positions[0];
        // net_value = collateral_value + deposit_value - debt_value = 3000 + 0 - 10000 = saturate(0)
        assert_eq!(pos.net_value, 0);
    }

    #[test]
    fn test_prediction_out_of_bounds_tier_h5() {
        let prices = make_prices();
        let market = PredictionMarketCellData {
            market_id: [0x33; 32],
            status: 0,
    ...
    #[test]
    fn test_summary_total_value_is_sum_of_all_h5() {
        let prices = make_prices();
        let mut builder = PortfolioBuilder::new([0x11; 32], prices);
        builder.add_token(token(1), 10 * PRECISION); // $30000
        builder.add_token(token(2), 5000 * PRECISION); // $5000
        let portfolio = builder.build();
        assert_eq!(portfolio.summary.total_value, portfolio.summary.token_value);
        assert_eq!(portfolio.summary.token_value, 35_000 * PRECISION);
    }

    #[test]
    fn test_lp_deposit_block_preserved_h5() {
        let prices = make_prices();
        let pool = make_pool(1, 2, 1000 * PRECISION, 3_000_000 * PRECISION);
        let lp = make_lp(100 * PRECISION, PRECISION);
        let mut builder = PortfolioBuilder::new([0x11; 32], prices);
        builder.add_lp(lp, pool);
        let portfolio = builder.build();
        assert_eq!(portfolio.lp_positions[0].deposit_block, 100);
    }

    #[test]
    fn test_estimate_portfolio_apy_no_earning_positions_h5() {
        // No insurance, no lending deposits → APY = 0
        let apy = estimate_portfolio_apy(&[], &[]);
        assert_eq!(apy, 0);
    }

    #[test]
    fn test_estimate_portfolio_apy_lending_only_h5() {
        let lending = vec![LendingPosition {
            pool_id: [0; 32],
            collateral_amount: 0,
            collateral_value: 0,
            debt_amount: 0,
            ...
            deposit_value: 1000 * PRECISION,
            net_value: 1000 * PRECISION,
        }];
        let apy = estimate_portfolio_apy(&[], &lending);
        assert_eq!(apy, 300); // 3% nominal APY for deposits
    }

    #[test]
    fn test_calculate_il_zero_entry_price_h5() {
        let pool = PoolCellData {
            reserve0: 1000 * PRECISION,
            reserve1: 3_000_000 * PRECISION,
            total_lp_supply: 1000 * PRECISION,
            fee_rate_bps: 30,
            twap_price_cum: 0,
            twap_last_block: 100,
            k_last: [0; 32],
            minimum_liquidity: 1000,
            pair_id: [0; 32],
            token0_type_hash: token(1),
            token1_type_hash: token(2),
        };
        let il = calculate_il(0, &pool, 3000 * PRECISION, PRECISION);
        assert_eq!(il, 0); // Zero entry price → no IL
    }

    #[test]
    fn test_builder_chaining_h5() {
        let prices = make_prices();
        let mut builder = PortfolioBuilder::new([0x11; 32], prices);
        let pool = make_pool(1, 2, 1000 * PRECISION, 3_000_000 * PRECISION);
        let lp = make_lp(100 * PRECISION, PRECISION);
        // Chaining should return &mut Self
        let b = builder.add_lp(lp, pool);
        b.add_token(token(1), 10 * PRECISION);
        let portfolio = b.build();
        assert_eq!(portfolio.lp_positions.len(), 1);
        assert_eq!(portfolio.token_balances.len(), 1);
    }

    #[test]
    fn test_multiple_lending_worst_hf_h5() {
        let prices = make_prices();
        let vault1 = make_vault(10 * PRECISION, 1000 * PRECISION); // Safe
        let vault2 = make_vault(1 * PRECISION, 5000 * PRECISION); // Risky
        let pool = make_lending_pool();
        let mut builder = PortfolioBuilder::new([0x11; 32], prices);
        builder.add_vault(vault1, pool.clone()).add_vault(vault2, pool);
        let portfolio = builder.build();
        // Worst HF should be the lower one (risky vault)
        assert!(portfolio.summary.worst_health_factor < u128::MAX);
        let hf1 = portfolio.lending_positions[0].health_factor;
        let hf2 = portfolio.lending_positions[1].health_factor;
        assert_eq!(portfolio.summary.worst_health_factor, hf1.min(hf2));
    }

    #[test]
    fn test_insurance_pool_id_preserved_h5() {
        let prices = make_prices();
        let ins = make_insurance();
        let expected_id = ins.pool_id;
        let mut builder = PortfolioBuilder::new([0x11; 32], prices);
        builder.add_insurance(ins, 1000 * PRECISION);
        let portfolio = builder.build();
        assert_eq!(portfolio.insurance_positions[0].pool_id, expected_id);
    }

    #[test]
    fn test_token_type_hash_preserved_h5() {
        let prices = make_prices();
        let mut builder = PortfolioBuilder::new([0x11; 32], prices);
        builder.add_token(token(3), 1 * PRECISION); // BTC
        let portfolio = builder.build();
        assert_eq!(portfolio.token_balances[0].token_type_hash, token(3));
    }

    #[test]
    fn test_summary_total_debt_h5() {
        let prices = make_prices();
        let vault = make_vault(10 * PRECISION, 5000 * PRECISION);
        let pool = make_lending_pool();
        let mut builder = PortfolioBuilder::new([0x11; 32], prices);
        builder.add_vault(vault, pool);
        let portfolio = builder.build();
        assert!(portfolio.summary.total_debt > 0);
        assert_eq!(portfolio.summary.total_debt, portfolio.lending_positions[0].debt_value);
    }
}
