...
const TOKEN_META = {
  ETH: { logo: '\u27E0', decimals: 4 },
  USDC: { logo: '$', decimals: 2 },
  USDT: { logo: '$', decimals: 2 },
  WBTC: { logo: '\u20BF', decimals: 6 },
}
...
const CHAIN_PAIRS = CHAINS.flatMap((src) =>
  CHAINS.filter((dst) => dst.id !== src.id).map((dst) => ({
...
function generateMockTransactions() {
  ...
  for (let i = 0; i < 15; i++) {
    ...
    const txHashBytes = Array.from({ length: 32 }, () =>
      Math.floor(rng() * 256).toString(16).padStart(2, '0')
    ).join('')
    const txHash = '0x' + txHashBytes

    const lzMsgIdBytes = Array.from({ length: 16 }, () =>
      Math.floor(rng() * 256).toString(16).padStart(2, '0')
    ).join('')
    const lzMessageId = '0x' + lzMsgIdBytes
...
const staggerContainer = {
  ...
  visible: {
    opacity: 1,
    transition: { staggerChildren: 0.06 * PHI, delayChildren: 0.1 },
  },
}
    ...
    <div className="flex items-center justify-between py-3 px-2">
      ...
      <div className="flex-1 mx-3 flex items-center relative">
        <div
          ...
          style={{ backgroundColor: src.hex + '40' }}
        />
        ...
        <div
          className="h-[2px] flex-1 rounded-full"
          style={{ backgroundColor: dst.hex + '40' }}
        />
      </div>
...
function TransactionRow({ tx, index, expanded, onToggle }) {
          ...
          <div className="flex items-center justify-between gap-3">
            ...
            <div className="flex items-center gap-3 min-w-0">
              ...
              <div className="min-w-0">
                ...
                <div className="text-[11px] text-black-400 truncate">
                  {tx.srcChain.name} &rarr; {tx.dstChain.name}
                </div>
              </div>
            </div>
            ...
            <div className="flex items-center gap-3 flex-shrink-0">
              ...
              <span
                className={`inline-flex items-center gap-1 px-2 py-0.5 rounded-full text-[10px] font-medium border ${sty.badge}`}
              >
                <div className={`w-1.5 h-1.5 rounded-full ${sty.dot}`} />
                {sty.label}
              </span>
              <motion.svg
              ...
              <div className="px-3 sm:px-4 pb-4 border-t border-black-700/50">
                ...
                <div className="grid grid-cols-2 gap-3 mt-2">
                  <div className="p-2.5 rounded-lg bg-black-900/50">
                    <div className="text-[10px] text-black-500 mb-0.5">Tx Hash</div>
                    <div className="text-xs font-mono text-black-300 flex items-center gap-1.5">
                      <span className="truncate">{truncHash(tx.txHash)}</span>
                      <button
                        onClick={(e) => {
                          e.stopPropagation()
                  ...
                  <div className="p-2.5 rounded-lg bg-black-900/50">
                    <div className="text-[10px] text-black-500 mb-0.5">LZ Message ID</div>
                    <div className="text-xs font-mono text-black-300 flex items-center gap-1.5">
                      <span className="truncate">{truncHash(tx.lzMessageId)}</span>
                      <button
                        onClick={(e) => {
                          e.stopPropagation()
                          navigator.clipboard.writeText(tx.lzMessageId)
                        }}
                        className="text-black-500 hover:text-black-200 transition-colors flex-shrink-0"
                        title="Copy LZ message ID"
                      >
                        <svg className="w-3 h-3" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                          <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M8 16H6a2 2 0 01-2-2V6a2 2 0 012-2h8a2 2 0 012 2v2m-6 12h8a2 2 0 002-2v-8a2 2 0 00-2-2h-8a2 2 0 00-2 2v8a2 2 0 002 2z" />
                        </svg>
                  ...
                  <div className="p-2.5 rounded-lg bg-black-900/50">
                    <div className="text-[10px] text-black-500 mb-0.5">Bridge Time</div>
                    <div className="text-xs text-black-300">
                      {tx.status === 'failed' ? '---' : fmtTime(tx.bridgeTimeSec)}
                    </div>
                  </div>

                  <div className="p-2.5 rounded-lg bg-black-900/50">
                    <div className="text-[10px] text-black-500 mb-0.5">Bridge Fee</div>
                    <div className="text-xs font-mono text-black-300">
                      {tx.lzFee} ETH
                    </div>
                  </div>
                </div>
                ...
                {tx.status === 'pending' && (
                  ...
                  >
                    <div className="flex items-center gap-2">
                      ...
                      <div>
                        <div className="text-xs font-medium" style={{ color: CYAN }}>
                          Estimated completion: ~{fmtTime(tx.etaSec)}
                        </div>
                        <div className="text-[10px] text-black-500">
                          LayerZero message in transit
                        </div>
                      </div>
                    </div>
                    {/* Progress bar */}
...
function BridgeHistoryPage() {
  ...
  const stats = useMemo(() => {
    ...
    const avgTime = confirmed.length > 0
      ? Math.round(confirmed.reduce((a, t) => a + t.bridgeTimeSec, 0) / confirmed.length)
      : 0
    const successRate = MOCK_TXS.length > 0
      ? ((confirmed.length / MOCK_TXS.length) * 100).toFixed(1)
      : '0.0'
    const totalFees = MOCK_TXS.reduce((a, t) => a + parseFloat(t.lzFee), 0)

    return {
      totalVolume,
      txCount: MOCK_TXS.length,
      avgTime,
      successRate,
  ...
  const filteredTxs = useMemo(() => {
    ...
    if (chainPairFilter) {
      list = list.filter(
        (t) =>
          t.srcChain.id === chainPairFilter.srcId &&
          t.dstChain.id === chainPairFilter.dstId
      )
    }
    ...
    if (dateRange !== 'all') {
      ...
      if (cutoff) {
        list = list.filter((t) => now - t.timestamp <= cutoff)
      }
    }
  ...
  return (
    <div className="max-w-4xl mx-auto px-4 pb-20">
      <PageHero
        ...
        subtitle="Track your cross-chain transfers powered by LayerZero V2"
        category="protocol"
        badge={pendingTxs.length > 0 ? `${pendingTxs.length} Pending` : undefined}
        badgeColor={CYAN}
      />
      ...
      >
        <GlassCard className="mb-6">
          <div className="grid grid-cols-2 sm:grid-cols-4 divide-x divide-black-700/50">
            <StatBox
              label="Total Bridged Volume"
              value={fmtUsd(stats.totalVolume)}
              sub={`${stats.txCount} transactions`}
            />
            <StatBox
              label="Transactions"
              value={stats.txCount}
      ...
      {pendingTxs.length > 0 && (
        ...
        >
          <div className="flex items-center gap-2 mb-3">
            <motion.div
              animate={{ rotate: 360 }}
              transition={{ repeat: Infinity, duration: PHI * 2, ease: 'linear' }}
            >
              <svg className="w-4 h-4" style={{ color: CYAN }} fill="none" viewBox="0 0 24 24" stroke="currentColor">
                <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M4 4v5h.582m15.356 2A8.001 8.001 0 004.582 9m0 0H9m11 11v-5h-.581m0 0a8.003 8.003 0 01-15.357-2m15.357 2H15" />
              </svg>
        ...
        <GlassCard className="p-4">
          ...
          <div className="grid grid-cols-3 gap-3">
            <div className="p-3 rounded-xl bg-black-900/50">
              ...
              <div className="text-[10px] text-black-500 mt-0.5">
                ~{fmtUsd(parseFloat(stats.totalFees) * 3500)}
              </div>
            </div>
            <div className="p-3 rounded-xl bg-black-900/50">
            ...
            <div className="p-3 rounded-xl bg-black-900/50">
              ...
              <div className="text-sm font-mono font-medium text-white">
                {(parseFloat(stats.totalFees) / stats.txCount).toFixed(4)} ETH
              </div>
      ...
      >
        <div className="flex items-center justify-between mb-3">
          <h2 className="text-sm font-semibold text-white">
            All Transactions
            <span className="ml-2 text-xs text-black-500 font-normal">
              ({filteredTxs.length})
            </span>
          </h2>
          <button
                ...
                <div className="space-y-4">
                  ...
                  <div>
                    ...
                    <div className="flex flex-wrap gap-2">
                      <FilterChip
                        label="All Pairs"
                        active={chainPairFilter === null}
                        onClick={() => setChainPairFilter(null)}
                      />
                      {CHAIN_PAIRS.slice(0, 8).map((cp) => (
                        <FilterChip
                          ...
                          label={cp.label}
                          active={
                            chainPairFilter?.srcId === cp.srcId &&
                            chainPairFilter?.dstId === cp.dstId
                          }
                          onClick={() =>
                            setChainPairFilter(
                              chainPairFilter?.srcId === cp.srcId &&
        ...
        >
          <GlassCard className="p-8">
            <div className="text-center">
              <svg
                className="w-12 h-12 mx-auto text-black-600 mb-3"
                fill="none"
                viewBox="0 0 24 24"
                stroke="currentColor"
              >
      ...
      >
        ...
        <div className="text-[10px] text-black-600 mt-1">
          0% protocol fees &middot; Immutable burn-and-mint transfers
        </div>
      </motion.div>
    </div>
  )
