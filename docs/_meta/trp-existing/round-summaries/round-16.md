...
| **R4 (cure)** | 5 findings fixed this cycle (F01, K01, K02, K03, K06) | 5/19 = 26% closure |
| **Integration** | 3 loop pairs confirmed cross-feeding | YES |

**Overall Grade: S** — All loops produced findings + integration across 3 pairs + cures applied.

**Note**: This round introduced the R4 cure phase (TRP Runner v2.0). Prior rounds were diagnosis-only. The 26% closure rate reflects the first application of the cure loop — future rounds should target >50%.
...
| Finding | Fix | Status |
|---------|-----|--------|
| **F01** (HIGH): Priority bids stuck in CRA | Added `withdrawPriorityBids()` to CRA + interface. `_forwardPriorityBids` in Core now pulls ETH from CRA before forwarding to treasury. | CLOSED |
| **K03** (HIGH): Three-way fee contradiction | Fixed NatSpec in BuybackEngine (marked as dormant, not actively receiving from FeeRouter) and ProtocolFeeAdapter (corrected flow to "100% to LPs via ShapleyDistributor"). | CLOSED |
| **K01** (HIGH): MIN_FEE_BPS comment wrong | Fixed comment: "1 bps = 0.01%" (was "0.5 bps = 0.005%"). | CLOSED |
| **K02** (HIGH): FeeController missing from catalogue | Added FeeController entry to AMM section of CONTRACTS_CATALOGUE.md + added to VibeAMM dependency list. | CLOSED |
| **K06** (MEDIUM): Catalogue counting error | Fixed "3 contracts" → "5 contracts" to match the 5 listed. | CLOSED |

**Closure rate**: 5/19 findings fixed (26%). First application of R4 cure phase.
...
### Unfixed R1 Findings
1. **F02** (HIGH): Reserve corruption in sequential batch execution — needs design decision (snapshot reserves vs document behavior)
2. **F03** (MEDIUM): Donation attack false positive — needs token transfer reordering
3. **F04** (MEDIUM): Block entropy predictability — validator/proposer edge case
4. **F06** (MEDIUM): BatchMath overflow for large reserves
5. **F07** (MEDIUM): _orderDepositors index desync on direct CRA reveals

### Unfixed R2 Findings
1. **K04** (MEDIUM): createPool() permissionless claim false for direct AMM calls
2. **K05** (MEDIUM): Non-atomic batch settlement undocumented
