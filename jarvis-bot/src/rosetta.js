...
const LEXICONS = {
  ...
  // Mirrored from frontend/src/utils/rosetta-engine.js (compiler-level persistence).
  vibeswap: {
    ...
    concepts: {
      ...
      'cave':             { universal: 'constraint_innovation',  desc: 'Constraints drive innovation. Patterns under limits scale when limits lift.' },
      ...
      'vsos':             { universal: 'capability_merger',      desc: 'Financial OS. Built-in first, plugin second. Lean core, modular extensions.' },
      ...
      'shapley':          { universal: 'fair_attribution',       desc: 'Marginal contribution across all coalitions. Math, not timestamp. The Cave Theorem.' },
      ...
      'absorb':           { universal: 'capability_merger',      desc: 'Mutualist absorption via Plugin Registry, Hooks, Modular Curves, Shared Insurance.' },
      'tokens':           { universal: 'committed_resources',    desc: 'VIBE = 21M governance. JUL = elastic PoW rebase. CKB-native = state rent.' },
      'pom':              { universal: 'fair_attribution',       desc: 'Proof of Mind. Human or AI. Contribution, not consciousness. Retroactive Shapley.' },
      'cincin':           { universal: 'threshold_gating',       desc: 'Cincinnatus Roadmap. Grade 0 to 5 disintermediation.' },
      'metasoc':          { universal: 'mutualistic_co_location', desc: 'Parasocial → meta-social. Mutual proportional value.' },
      'boot':             { universal: 'connection_init',        desc: 'WAL check → CKB → SESSION_STATE → git pull → Ready.' },
      'work':             { universal: 'automation_pipeline',    desc: 'PCP gate → Execute → Verify → Commit → Push.' },
      'auto':             { universal: 'automation_pipeline',    desc: 'Run IT mode. Instant. BIG-SMALL rotation. Parallel agents.' },
      'reboot':           { universal: 'cyclic_renewal',         desc: '50% context boundary. Stop, commit, push, fresh session.' },
      'end':              { universal: 'rhythmic_closure',       desc: 'Block header → commit → push to origin. Mandatory.' },
      'crash':            { universal: 'corrective_action',      desc: 'WAL.md ACTIVE → manifest → cross-ref git → auto-commit orphans.' },
      'trp':              { universal: 'systematic_review',      desc: 'Recursive self-improvement. R0 compress → R1 adversarial → R2 knowledge → R3 capability.' },
      'cto':              { universal: 'priority_under_constraint', desc: 'Mitosis k=1.3, cap 5. RAM <60% spawn, 60-80% hold, >80% kill.' },
      'jarvis_agent':     { universal: 'capability_hierarchy',   desc: 'Haiku lightweight, Sonnet medium, Opus heavy. Shards over swarms.' },
      'frank':            { universal: 'compress_context',       desc: 'Direct. Results over process. Match energy. No tips, no farming.' },
      'discret':          { universal: 'authority_boundary',     desc: 'No personal details in public repos. Patience with community.' },
      'two_loop':         { universal: 'concept_portability',    desc: 'Code inspires text inspires code. Compounding round-trip.' },
      'skill':            { universal: 'capability_hierarchy',   desc: 'Mistakes → Skills → SKB. Lazy init, name-first search.' },
      'hiero':            { universal: 'compress_context',       desc: 'Memory as logic primitive, not prose. Glyphs over paragraphs.' },
      'ilws':             { universal: 'transmission_method',    desc: 'CKB edits behave as hand-written LoRA. Instructions outperform RAG.' },
      'rlm':              { universal: 'upward_delegation',      desc: 'Recursive sub-LLM delegation outperforms summarization.' },
      'ksize':            { universal: 'pattern_generalization', desc: 'Knowledge access beats raw model size. The CKB thesis.' },
      'ctxeng':           { universal: 'pattern_generalization', desc: 'Context engineering. Industry name for what we do.' },
      'mirix':            { universal: 'persistent_state',       desc: 'Six memory types. Five matched.' },
      'deep_funding':     { universal: 'fair_attribution',       desc: 'Log-space scoring + human jury + dependency DAG credit flow.' },
      'x402':             { universal: 'interface',              desc: 'HTTP 402 micropayments.' },
      'erc8004':          { universal: 'trust_score',            desc: 'On-chain trust identity. Portable reputation.' },
    },
  },
  rosetta: {
    ...
    concepts: {
      ...
      'glyph':                { universal: 'compress_context',        desc: 'Compressed compiled form. Density × stability × pointer-deref.' },
      ...
      'covenant':             { universal: 'unchanging_constraint',   desc: "Tet's Law. Immutable inter-agent rules. The protocol's bedrock." },
      'compile':              { universal: 'transmission_method',     desc: 'CKG op: convert source-form into universal-anchored compiled form.' },
      'decompile':            { universal: 'adaptive_delivery',       desc: 'CKG op: project compiled form into a target language, adapting surface to receiver.' },
      'projection':           { universal: 'pattern_generalization',  desc: 'One compiled meaning, viewed through one of N language surfaces.' },
      'universal_index':      { universal: 'established_pattern',     desc: 'Reverse map: universal → all terms anchoring to it across all lexicons.' },
      'semantic_drift':       { universal: 'semantic_drift',          desc: 'Meaning loss accumulated across translation chains.' },
      ...
      'ckg':                  { universal: 'knowledge_fusion',        desc: 'Compiled Knowledge Graph. CKB (storage) / CKA (architecture) / CKG (meaning-layer).' },
      'compiler_persistence': { universal: 'persistent_state',        desc: 'Meaning survives substrate at the compiled level.' },
      'composition':          { universal: 'capability_merger',       desc: 'CKG op: link multiple lexicons into one combined object.' },
      'gap_discovery':        { universal: 'anomaly',                 desc: "CKG op: detect terms whose universal isn't yet in the symtab." },
      'similarity':           { universal: 'co_movement',             desc: 'CKG op: distance between two compiled forms in universal-concept space.' },
      'bootstrap':            { universal: 'connection_init',         desc: 'Describing the compiler in its own terms.' },
    },
  },
  usd8: {
    ...
    concepts: {
      ...
      'peg_anchor':           { universal: 'stable_equilibrium',      desc: 'The 1:1 USD anchor. Structurally enforced.' },
      'brevis_proof':         { universal: 'integrity_proof',         desc: 'ZK-verifiable proof of correct distribution.' },
      'holder':               { universal: 'committed_resources',     desc: 'An address holding USD8. The unit of fair-attribution participation.' },
      'issuer':               { universal: 'assigned_responsibility', desc: 'The entity that mints USD8. Not the yield owner.' },
      'redemption':           { universal: 'corrective_action',       desc: 'Conversion of USD8 back to underlying USD.' },
      'depeg':                { universal: 'environment_induced_drift', desc: 'Loss of 1:1 peg under stress.' },
      'transparency':         { universal: 'systematic_review',       desc: 'Public reserve attestation. Auditable on-chain.' },
      'yield_routing':        { universal: 'automation_pipeline',     desc: 'Programmatic distribution of reserve earnings.' },
      'augmented_stable':     { universal: 'stable_equilibrium',      desc: 'Stablecoin fairness as structural property — not a promise.' },
    },
  },
  amd: {
    ...
    concepts: {
      ...
      'structural_invariant': { universal: 'unchanging_constraint',   desc: 'A property the math guarantees regardless of context.' },
      'mechanism':            { universal: 'established_pattern',     desc: 'A market or coordination process with defined rules and outcomes.' },
      'fairness_axiom':       { universal: 'foundational_axiom',      desc: 'A bedrock fairness rule from which mechanism legitimacy descends.' },
      'substrate_match':      { universal: 'pattern_generalization',  desc: 'Mechanism geometry must match substrate geometry. As above, so below.' },
      'fractal_market':       { universal: 'pattern_generalization',  desc: 'Power-law / scale-invariant market structure.' },
      'golden_ratio':         { universal: 'pattern_generalization',  desc: 'Phi-based scaling. The substrate-matched proportionality.' },
      'fibonacci_scale':      { universal: 'pattern_generalization',  desc: 'Throughput limits arranged on Fibonacci retracement levels.' },
      'augment_not_replace':  { universal: 'voluntary_agreement',     desc: 'AMD changes which outcomes are reachable, not whether outcomes happen.' },
      'math_enforcement':     { universal: 'integrity_proof',         desc: 'Properties guaranteed by math, not trusted parties.' },
      'shapley_distribution': { universal: 'fair_attribution',        desc: 'Per-coalition marginal contribution allocation.' },
  ...
  agov: {
    ...
    concepts: {
      'physics_layer':        { universal: 'unchanging_constraint',   desc: 'Math-enforced invariants (P-000, P-001). Not mutable.' },
      'constitution_layer':   { universal: 'foundational_axiom',      desc: 'Charter-level rules below math but above operational governance.' },
      'governance_layer':     { universal: 'collective_decision',     desc: 'DAO votes, operational policy. Free within physics + constitution.' },
      'hierarchy':            { universal: 'capability_hierarchy',    desc: 'Physics > Constitution > Governance.' },
      'capture_resistance':   { universal: 'defensibility',           desc: 'Even if rent-seekers capture the DAO, they cannot override math.' },
      'vote_within_bounds':   { universal: 'voluntary_agreement',     desc: 'DAO operates freely within math-enforced bounds.' },
      'math_court':           { universal: 'integrity_proof',         desc: 'Math itself adjudicates governance validity.' },
      'override_impossible':  { universal: 'unchanging_constraint',   desc: 'Properties the protocol cannot un-do via any governance route.' },
      'governance_capture':   { universal: 'extraction_rent',         desc: 'Failure mode: rent-seekers control DAO and tilt rules.' },
      'separation_of_powers': { universal: 'authority_boundary',      desc: 'Each layer has a defined scope.' },
      'p_000':                { universal: 'foundational_axiom',      desc: 'Fairness Above All. Load-bearing constitutional axiom.' },
...
//   Medicine, Music, Law, Education, Engineering, Agriculture,
//   Biology, Physics, Psychology, Philosophy, Economics (macro)
//
...
export const EXTENDED_UNIVERSAL_CONCEPTS = {
  ...
  'stable_equilibrium':         'Self-regulating balance maintained against external pressure — homeostasis',
  ...
  'threat_actor':               'An agent that causes harm by entering and disrupting the system',
  'coupled_risk':               'Two problems that co-occur and amplify each other',
  'temporary_recovery':         'Condition has retreated — not necessarily resolved',
  'voluntary_agreement':        'Participant understands the stakes and freely consents',
  'expectation_effect':         'Improvement driven by belief rather than the intervention itself',
  'preventive_action':          'Acting before harm occurs to prevent it — prophylaxis',
  'outcome_forecast':           'Predicting the likely trajectory of a condition over time',
  'root_cause':                 'The origin or underlying cause — etiology',

  // ---- Law / Authority ----
  'established_pattern':        'A prior case or practice that shapes how similar situations are handled',
  'authority_boundary':         'The domain within which a rule-maker\'s rules apply',
  'assigned_responsibility':    'Who bears the cost when something goes wrong — liability',
  'civil_harm':                 'A wrong causing damage outside of contract — tort',
  'prior_commitment_lock':      'Cannot contradict past position to harm someone who relied on it — estoppel',
  'corrective_action':          'What the wronged party receives to make them whole — remedy',
  'right_to_participate':       'Having enough at stake to bring a claim — standing',
  'evidence_threshold':         'How much evidence is required to satisfy a burden of proof',
  'pre_commitment_audit':       'Thorough investigation before entering an agreement — due diligence',
  'trust_obligation':           'A duty to act in another\'s best interest above your own — fiduciary',
  'known_incompatibility':      'A condition that makes a normally helpful action harmful instead',

  // ---- Engineering / Physical ----
  'acceptable_variance':        'How much deviation from spec is allowed before failure — tolerance / slippage',
  'critical_dependency':        'A component whose failure brings down the whole structure — load-bearing',
  'lateral_pressure':           'Force applied parallel to a surface — the sliding kind of failure',
  'accumulated_degradation':    'Failure from repeated sub-limit stress over time — fatigue',
  'environment_induced_drift':  'Change in shape or behavior caused by ambient conditions',
  'elastic_limit':              'The point past which deformation becomes permanent — yield strength',
  'backup_capacity':            'Parallel systems so one failure doesn\'t cause total collapse — redundancy',
  'maximum_load':               'The most a system can handle before breaking — tensile strength',
  'margin_of_safety':           'Building to withstand more than expected — safety factor',
  'amplification_at_frequency': 'When external rhythm matches internal rhythm and energy builds dangerously',
  'slow_permanent_drift':       'Gradual deformation under sustained load over time — creep',
  'priority_under_constraint':  'Deciding what to address first when resources are scarce — triage / IPM',

  // ---- Education / Learning ----
  'structured_support':         'Temporary scaffolding enabling work the learner can\'t yet do alone',
  'evaluation_framework':       'Explicit criteria making assessment transparent — rubric',
  'adaptive_delivery':          'Adjusting approach for individual learners — differentiation',
  'in_progress_feedback':       'Assessment during the process, not just at the end — formative',
  'capability_hierarchy':       'A ranked ladder of cognitive or functional abilities — Bloom\'s taxonomy',
  'transmission_method':        'The theory and practice of how knowledge moves between minds — pedagogy',
  'thinking_about_thinking':    'Awareness of one\'s own reasoning process — metacognition',
  'growth_edge':                'What you can do with help but not alone — zone of proximal development',
  'threshold_gating':           'Requiring demonstrated competence before advancement — mastery learning',
  'concept_portability':        'Applying knowledge from one domain to solve problems in another — transfer',

  // ---- Music / Pattern ----
  'productive_tension':         'Friction that demands resolution — dissonance, creative conflict',
  'tension_release':            'Movement from instability to a stable state — resolution, closing',
  'independent_parallel_lines': 'Two streams moving autonomously but creating coherence together — counterpoint',
  'identity_signature':         'The quality that makes something recognizable as itself — timbre, fingerprint',
  'rhythmic_closure':           'A sequence that signals an ending or resting point — cadence',
  'unexpected_emphasis':        'Placing stress where the pattern doesn\'t expect it — syncopation',
  'recurring_unit':             'A small pattern that repeats and builds meaning — motif, primitive',
  'intensity_modulation':       'Variation in force to create expression — dynamics, emphasis',
  'execution_rate':             'The speed at which events unfold — tempo, throughput',
  'operating_context':          'The home base that gives all other elements their meaning — key, frame',

  // ---- Agriculture / Stewardship ----
  'cyclic_renewal':             'Rotating what occupies a space so the previous use\'s depletion recovers',
  'substrate_quality':          'The underlying conditions that determine what can grow on top — soil health',
  'capability_merger':          'Joining two entities so each provides what the other lacks — grafting',
  'prerequisite_condition':     'Something that must be experienced before a capability unlocks — vernalization',
  'context_fingerprint':        'How place of origin is inseparable from what something is — terroir',
  'intentional_rest':           'Leaving a resource idle so it recovers and regenerates — fallow',
  'mutualistic_co_location':    'Placing complementary things together so each helps the other — companion planting',
  'graduated_exposure':         'Slowly introducing stress to build tolerance before full deployment — hardening off',
