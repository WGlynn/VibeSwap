...
export default function GovernancePage() {
  ...
  return (
    <div className="max-w-3xl mx-auto px-4 py-6">
      ...
      <Section index={0} title="Overview" subtitle="Current governance state">
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
          {[
            { label: 'Active Proposals', value: activeProposals.length, accent: true },
            { label: 'Quorum', value: '62%' },
            { label: 'Your Voting Power', value: isConnected ? userPower.total.toLocaleString() : '--' },
            { label: 'Delegation', value: isConnected ? 'Self' : '--' },
          ].map((s) => (
          ...
          {PROPOSALS.map((p) => {
            ...
            return (
              <div key={p.id} className="rounded-xl border border-black-700/30 bg-black-900/30 p-4">
                ...
                {/* 12. Animated vote progress bars */}
                ...
                {/* 4. Vote casting: For / Against / Abstain with confirmation */}
        ...
        {isConnected ? (
          <div className="space-y-3">
            ...
            <div className="flex gap-2">
              ...
              <button disabled={!delegateAddr} className="px-4 py-2 rounded-lg text-xs font-mono font-bold transition-all"
                style={delegateAddr ? { background: CYAN, color: '#000', boxShadow: `0 0 16px ${CYAN}30` } : { background: '#333', color: '#666' }}>Delegate</button>
            </div>
            <div className="text-[10px] font-mono text-black-500">Delegating transfers your voting weight but not your tokens. Revoke at any time.</div>
          </div>
        ...
        {isConnected && userPower.total >= 10000 ? (
          !showCreateForm ? (
            ...
            <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="space-y-3">
              ...
              <div className="flex gap-2">
                <button onClick={() => setShowCreateForm(false)} className="flex-1 py-2 rounded-lg text-xs font-mono text-black-400 border border-black-700/40 hover:bg-black-800/50">Cancel</button>
