...
// ============ Status Helpers ============
const STATUS_CONFIG = {
  completed: { label: 'Completed', color: MATRIX_GREEN, bg: `${MATRIX_GREEN}15` },
...
const MILESTONE_ICON = {
  done: { symbol: '\u2713', color: MATRIX_GREEN },
  'in-progress': { symbol: null, color: CYAN },
  pending: { symbol: '\u25CB', color: 'rgba(255,255,255,0.2)' },
}
...
function MilestoneIcon({ status }) {
  const config = MILESTONE_ICON[status]
  if (status === 'in-progress') {
    return <Spinner color={config.color} size={16} />
  }
  return (
...
function StatusBadge({ status }) {
  ...
  return (
    <div
      className="flex items-center gap-2 text-xs font-mono px-3 py-1 rounded-full border border-white/5"
...
function TimelineNode({ phase, index, expandedId, setExpandedId }) {
  ...
  const completedCount = phase.milestones.filter((m) => m.status === 'done').length
  const inProgressCount = phase.milestones.filter((m) => m.status === 'in-progress').length
  const progressPct = Math.round(
    ((completedCount + inProgressCount * 0.5) / phase.milestones.length) * 100
  )
  ...
  return (
    <motion.div variants={itemVariants} className="relative flex gap-4 sm:gap-6">
      ...
      <div className="flex flex-col items-center flex-shrink-0">
        ...
        <motion.button
          ...
          style={{
            ...
            backgroundColor: isDone ? `${config.color}20` : 'rgba(0,0,0,0.6)',
        ...
        >
          {isDone ? (
            ...
            <span style={{ color: config.color }}>{phase.id}</span>
          )}
          {isActive && (
            <motion.div
        ...
        {index < PHASES.length - 1 && (
          ...
          >
            <defs>
              <linearGradient
                id={`line-grad-${phase.id}`}
                x1="0"
                y1="0"
                x2="0"
                y2="1"
              >
                <stop offset="0%" stopColor={`${config.color}60`} />
                <stop
                  offset="100%"
                  stopColor={`${STATUS_CONFIG[PHASES[index + 1].status].color}30`}
                />
              </linearGradient>
            </defs>
            <rect
      ...
      <div className="flex-1 pb-6">
        <GlassCard
          glowColor={isActive ? 'terminal' : isDone ? 'matrix' : 'none'}
          hover={true}
          className="p-0"
        >
              ...
              <div className="relative w-12 h-12 flex-shrink-0">
                <svg className="w-12 h-12 -rotate-90" viewBox="0 0 36 36">
                  ...
                  <motion.circle
                    ...
                    whileInView={{ strokeDasharray: `${progressPct * 0.88} 88` }}
                    viewport={{ once: true }}
...
export default function RoadmapPage() {
  ...
  const currentPhaseCompleted = currentPhase
    ? currentPhase.milestones.filter((m) => m.status === 'done').length
    : 0
  const currentPhaseTotal = currentPhase ? currentPhase.milestones.length : 1
  const currentPhasePct = Math.round((currentPhaseCompleted / currentPhaseTotal) * 100)
  const totalVotes = COMMUNITY_REQUESTS.reduce((sum, r) => sum + r.votes, 0)
  ...
  return (
    <div className="min-h-screen pb-24">
      <PageHero
        category="system"
        title="Roadmap"
        subtitle="Building the future of fair exchange"
        badge={currentPhase ? `Phase ${currentPhase.id}` : undefined}
        badgeColor={CYAN}
      />
      ...
      <div className="max-w-4xl mx-auto px-4">
              ...
              <div className="flex items-center gap-4">
                ...
                <div>
                  ...
                  <div className="text-xl font-bold tracking-tight">
                    Phase 2:{' '}
                    <span style={{ color: CYAN }}>Expansion</span>
                  </div>
                </div>
              </div>
              <div className="flex items-center gap-4">
            ...
            <div className="mt-4">
              <div className="flex justify-between mb-1.5">
                <span className="text-[10px] font-mono text-white/30">
                  {currentPhaseCompleted}/{currentPhaseTotal} milestones
                </span>
              ...
              <div className="h-2 rounded-full bg-white/5 overflow-hidden">
                <motion.div
                  className="h-full rounded-full"
                  style={{
                    background: `linear-gradient(90deg, ${MATRIX_GREEN}, ${CYAN})`,
                  }}
                  initial={{ width: 0 }}
                  animate={{ width: '65%' }}
        ...
        >
          <GlassCard glowColor="terminal" className="p-6 mt-4 mb-8">
            <div className="flex items-center gap-3 mb-5">
              ...
              <div>
                ...
                <p className="text-xs text-white/40">
                  Top voted features — signal what matters most
                </p>
              </div>
            </div>
                    ...
                    <div className="relative px-4 py-3 flex items-center justify-between">
                      <div className="flex items-center gap-3">
                        <div
                          ...
                          style={{
                            borderColor: isVoted ? CYAN : 'rgba(255,255,255,0.2)',
                          }}
                        >
                          {isVoted && (
                            <motion.div
                      ...
                      <div className="flex items-center gap-3">
                        <span className="text-xs font-mono text-white/30">
                          {(req.votes + (isVoted ? 1 : 0)).toLocaleString()} votes
                        </span>
                        <span
                          className="text-xs font-mono font-bold"
                          style={{ color: CYAN }}
            ...
            {votedId && (
              ...
              >
                Your signal has been recorded. On-chain governance coming in Phase 4:
                Sovereignty.
              </motion.p>
            )}
          </GlassCard>
          ...
          <GlassCard glowColor="matrix" className="p-6 mb-8">
            <div className="flex items-center gap-3 mb-5">
              ...
              <div>
                ...
                <p className="text-xs text-white/40">
                  Latest completed milestones across all phases
                </p>
              </div>
            </div>
            <div className="space-y-2">
              {RECENT_MILESTONES.map((milestone, i) => (
                <motion.div
                  key={i}
                  ...
                  style={{
                    backgroundColor: `${MATRIX_GREEN}06`,
                    borderColor: `${MATRIX_GREEN}12`,
                  }}
                >
                  <span style={{ color: MATRIX_GREEN }} className="text-sm flex-shrink-0">
                    {'\u2713'}
                  </span>
                  <span className="text-sm text-white/70 flex-1">{milestone.text}</span>
        ...
        >
          ...
          <p className="text-sm text-white/20 font-mono">
            Building the future of fair exchange — one phase at a time
          </p>
