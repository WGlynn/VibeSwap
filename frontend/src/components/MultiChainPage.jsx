...
const TOKENS = [
  ...
  { symbol: 'UNI',   name: 'Uniswap',          logo: '\U0001f984' },
  ...
  return CHAINS.map(chain => {
    ...
    const tokens = selected.map(token => {
      ...
      const balance = token.symbol === 'ETH' ? rng() * 8 + 0.1
        ...
        : token.symbol === 'USDC' || token.symbol === 'USDT' || token.symbol === 'DAI' ? rng() * 15000 + 100
...
function TokenRow({ token, chainId }) {
    ...
    <motion.tr variants={fadeUp} className="border-b border-white/5 last:border-0 hover:bg-white/[0.02] transition-colors">
      <td className="py-3 px-3">
        <div className="flex items-center gap-2.5">
          <div className="w-7 h-7 rounded-full bg-white/5 flex items-center justify-center text-sm font-medium">
            {token.logo}
          </div>
          <div>
            <div className="text-sm font-medium text-white">{token.symbol}</div>
            <div className="text-[11px] text-gray-500">{token.name}</div>
          </div>
        </div>
      </td>
      <td className="py-3 px-3 text-right">
        <div className="text-sm font-mono text-white">{fmtBal(token.balance)}</div>
        <div className="text-[11px] text-gray-500 font-mono">${token.price.toLocaleString(undefined, { minimumFractionDigits: 2, maximumFractionDigits: 2 })}</div>
      </td>
      <td className="py-3 px-3 text-right">
        <div className="text-sm font-mono text-white">{fmt(token.value)}</div>
      </td>
      <td className="py-3 px-3 text-right">
      ...
      <td className="py-3 px-3 text-right">
        <div className="flex items-center justify-end gap-2">
          <Link
            to="/bridge"
            className="px-2.5 py-1 text-[11px] font-medium rounded-lg bg-cyan-500/10 text-cyan-400 hover:bg-cyan-500/20 border border-cyan-500/20 transition-colors"
          >
            Bridge
          </Link>
          <Link
            to="/swap"
            className="px-2.5 py-1 text-[11px] font-medium rounded-lg bg-green-500/10 text-green-400 hover:bg-green-500/20 border border-green-500/20 transition-colors"
          >
            Swap
          </Link>
        </div>
...
function ChainSection({ chain, isExpanded, onToggle, index }) {
        ...
        >
          ...
          <div className="flex items-center gap-4">
            <div className="text-right">
              ...
              <div className={`text-[11px] font-mono ${isPositive ? 'text-green-400' : 'text-red-400'}`}>
                {fmtPct(chain.change24h)}
              </div>
            </div>
            <motion.div
                ...
                <table className="w-full">
                  ...
                  <motion.tbody variants={stagger} initial="hidden" animate="show">
                    {chain.tokens.map(token => (
                      <TokenRow key={token.symbol} token={token} chainId={chain.id} />
                    ))}
                  </motion.tbody>
                </table>
              </div>
...
function BridgeRouteCard({ route, index }) {
  return (
    <motion.div variants={fadeUp}>
      <GlassCard className="p-4 hover:border-cyan-500/20 transition-colors cursor-pointer">
        <div className="flex items-center justify-between mb-3">
          <div className="flex items-center gap-2">
            <div
              ...
              style={{ backgroundColor: route.fromHex + '22', color: route.fromHex }}
            >
              {route.fromLogo}
            </div>
            ...
            <div
              className="w-6 h-6 rounded-full flex items-center justify-center text-[11px] font-bold"
              style={{ backgroundColor: route.toHex + '22', color: route.toHex }}
            >
              {route.toLogo}
            </div>
          </div>
          <div className="text-[10px] font-mono text-gray-500">
            Vol: {fmt(route.volume24h)}
          </div>
        </div>
        ...
        <Link
          to="/bridge"
          className="mt-3 block text-center py-1.5 text-[11px] font-medium rounded-lg bg-cyan-500/10 text-cyan-400 hover:bg-cyan-500/20 border border-cyan-500/20 transition-colors"
        >
          Bridge Now
        </Link>
      </GlassCard>
...
export default function MultiChainPage() {
  ...
  const chainAssets = useMemo(() => {
    ...
    return generateChainAssets(rng)
  }, [])

  const totalPortfolioValue = useMemo(
    () => chainAssets.reduce((sum, c) => sum + c.totalValue, 0),
    [chainAssets]
  )

  const totalChange24h = useMemo(() => {
    const weightedSum = chainAssets.reduce((sum, c) => sum + c.totalValue * c.change24h / 100, 0)
    return (weightedSum / totalPortfolioValue) * 100
  }, [chainAssets, totalPortfolioValue])

  const totalChange24hUsd = useMemo(
    () => totalPortfolioValue * totalChange24h / 100,
    [totalPortfolioValue, totalChange24h]
  )

  const sortedChains = useMemo(
    () => [...chainAssets].sort((a, b) => b.totalValue - a.totalValue),
    [chainAssets]
  )
  ...
  function expandAll() {
    const all = {}
    CHAINS.forEach(c => { all[c.id] = true })
    setExpandedChains(all)
  }
  ...
  const filteredRoutes = useMemo(() => {
    ...
    return POPULAR_ROUTES.filter(
      r => r.from.toLowerCase() === routeFilter || r.to.toLowerCase() === routeFilter
    )
  }, [routeFilter])
  ...
  if (!isConnected) {
    return (
      <div className="min-h-screen">
        <PageHero
          title="Multi-Chain"
          subtitle="Unified view of your assets across all supported networks via LayerZero"
          category="account"
        />
          ...
          <GlassCard className="p-12 text-center">
            ...
            <div className="flex flex-wrap justify-center gap-3">
              {CHAINS.map(chain => (
                <div
                  key={chain.id}
                  className="flex items-center gap-1.5 px-3 py-1.5 rounded-full text-xs border border-white/10 bg-white/5"
                >
            ...
            <div className="flex flex-col sm:flex-row sm:items-end sm:justify-between gap-4">
              <div>
                ...
                <div className="flex items-center gap-2 mt-1.5">
                  <span className={`text-sm font-mono ${isPositiveTotal ? 'text-green-400' : 'text-red-400'}`}>
                    {fmtPct(totalChange24h)}
                  </span>
                  <span className={`text-sm font-mono ${isPositiveTotal ? 'text-green-400/60' : 'text-red-400/60'}`}>
                    ({isPositiveTotal ? '+' : ''}{fmt(totalChange24hUsd)})
                  </span>
        ...
        <motion.div variants={fadeUp}>
            ...
            <div className="flex items-center gap-1 bg-white/5 rounded-lg p-0.5">
              {['all', 'ethereum', 'arbitrum', 'base'].map(f => (
                <button
                  ...
                  className={`px-2.5 py-1 text-[11px] font-mono rounded-md transition-colors capitalize ${
                    routeFilter === f
                      ? 'bg-cyan-500/20 text-cyan-400'
                      : 'text-gray-500 hover:text-gray-300'
                  }`}
          ...
          >
            {filteredRoutes.map((route, i) => (
              <BridgeRouteCard key={`${route.from}-${route.to}`} route={route} index={i} />
            ))}
          </motion.div>
        </motion.div>
          ...
          <GlassCard className="p-6" glowColor="terminal">
            <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-6 gap-6">
              <StatBlock
                ...
                value={analytics.totalBridges}
                sub="Lifetime"
                color={CYAN}
              />
              <StatBlock
                label="Gas Saved"
                value={fmt(analytics.gasSaved)}
                sub="vs individual txs"
                color="#22c55e"
              />
              <StatBlock
                label="Chains Used"
                value={`${analytics.uniqueChains}/6`}
                sub="Networks"
                color="#a78bfa"
              />
              <StatBlock
                label="Bridge Volume"
                value={fmt(analytics.totalBridgeVolume)}
                sub="Total bridged"
                color="#f59e0b"
              />
              <StatBlock
                label="Avg Bridge Time"
                value={analytics.avgBridgeTime}
                sub="LayerZero V2"
                color="#38bdf8"
              />
              <StatBlock
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <div className="flex items-center gap-4 mb-4">
                    <div className="flex-1 bg-white/5 rounded-lg p-3">
                      ...
                      <div className="text-[11px] font-mono text-amber-400">
                        {rebalanceSuggestion.fromPct.toFixed(1)}% of portfolio
                      </div>
                    </div>
                    ...
                    <div className="flex-1 bg-white/5 rounded-lg p-3">
                      ...
                      <div className="text-[11px] font-mono text-green-400">
                        {rebalanceSuggestion.toPct.toFixed(1)}% of portfolio
                      </div>
                    </div>
                  </div>
                  ...
                  <div className="flex items-center gap-6 text-xs text-gray-400">
                    <div className="flex items-center gap-1.5">
                      ...
                      <span>Save {fmt(rebalanceSuggestion.gasSavings)}/mo in gas</span>
                    </div>
                    <div className="flex items-center gap-1.5">
                      <span className="text-green-400">\u2713</span>
                      <span>Faster settlements on L2</span>
                    </div>
                    <div className="flex items-center gap-1.5">
                      <span className="text-green-400">\u2713</span>
                      <span>Lower slippage on Arbitrum pools</span>
                    </div>
                  </div>
                  ...
                  <div className="flex items-center gap-3 mt-4">
                    <Link
                      to="/bridge"
                      className="px-4 py-2 text-sm font-medium rounded-xl bg-cyan-500/20 text-cyan-400 hover:bg-cyan-500/30 border border-cyan-500/30 transition-colors"
                    >
                      Start Rebalance
                    </Link>
                    <button className="px-4 py-2 text-sm font-medium rounded-xl text-gray-500 hover:text-gray-300 transition-colors">
                      Dismiss
                    </button>
                  </div>
                </div>
