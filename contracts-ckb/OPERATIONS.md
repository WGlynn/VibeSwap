...
Five is large enough to lose two without halting and small enough that Will can personally know every operator. The two new validators are recruited from the pool of collaborators who have demonstrated technical engagement with the architecture (read a spec doc, raised a substantive issue, contributed a PR) AND have the operational capacity to run a node on their own hardware or cloud account. Candidate pool spans the existing collaborator graph (TRION-side, Pragma-side, Odysseus-Felix-circle, plus open call). No collaborator is pre-committed as of this writing; the doc commits the slot-count, not the names. The Phase-1 set is **not open**, it is **invited**.
